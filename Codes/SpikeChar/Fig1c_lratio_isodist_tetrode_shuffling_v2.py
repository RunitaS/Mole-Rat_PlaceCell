...
def process_folder(folder: str):
    ...
    return {'ntt_path': ntt_path, 'folder': folder, 'rows': rows}
...
# ── Summary histogram — every cell, pooled across all .ntt files ─────────────

def _clip_for_display(values: np.ndarray, upper_pct: float = 99.0):
    """Clip a long right tail at the given percentile so a handful of extreme
    clusters don't compress the histogram into one bin. Returns the clipped
    array and the clip ceiling (NaN-safe)."""
    finite = values[np.isfinite(values)]
    if finite.size == 0:
        return values, np.nan
    ceiling = np.percentile(finite, upper_pct)
    return np.clip(values, None, ceiling), ceiling


def plot_summary_histograms(all_results: list, root_dir: str):
    """Pool every cluster from every processed .ntt file and plot L-ratio and
    Isolation Distance histograms, split by whether the cluster passed the
    ...
    for res in all_results:
        if res is None:
            continue
        for row in res['rows']:
            lr, iso, shuf = row['l_ratio'], row['iso_dist'], row['shuffle']
            if np.isnan(lr) or np.isnan(iso):
                continue
            n_cells += 1
            p_lr, p_iso = shuf['p_lratio'], shuf['p_isodist']
            passed = (not np.isnan(p_lr) and not np.isnan(p_iso) and
                      p_lr < SIG_ALPHA and p_iso < SIG_ALPHA)
            if passed:
                lr_pass.append(lr)
                iso_pass.append(iso)
    ...
    if n_cells == 0:
        print('\nNo cells with valid L-ratio/IsoDist found — skipping summary histogram.')
        return
...
def main(root_dir: str = ROOT_DIR):
    ...
    plot_summary_histograms(all_results, root_dir)

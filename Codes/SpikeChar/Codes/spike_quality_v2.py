...
Metrics computed
────────────────
  SNR                  – A_peak / σ_noise  (MAD-based: σ = MAD/0.6745 on first-3+last-3-sample baseline)
...
def compute_snr_mad(waveforms: np.ndarray) -> dict:
    ...
    MAD-based noise estimate from the first-3+last-3-sample baseline.
    ...
    baseline = np.concatenate([wf[best_ch, :3, :], wf[best_ch, -3:, :]], axis=0).flatten()  # first 3 + last 3 samples as noise proxy

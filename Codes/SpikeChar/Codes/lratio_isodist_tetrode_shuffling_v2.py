...
For every .ntt file found anywhere under ROOT_DIR the script:
  ...
  2. Builds the tetrode feature matrix FD: for each of the 4 channels, one
     Energy value (feature_Energy.m) and one Peak value (feature_Peak.m —
     the maximum raw waveform amplitude on that channel), concatenated into
     a single (n_spikes, 8) matrix — the same shape of FD that
     CalculateLRatio_and_IsolationDistance.m builds before calling
     IsolationDistance.m / L_Ratio.m, but using Peak instead of WavePC1 for
     the second feature per channel.
...
feature_Energy.m + feature_Peak.m
  Per channel:
      Energy   = sqrt(sum(w², samples)) / sqrt(nSamples)
      Peak     = max(w, samples)                  # raw waveform peak amplitude
  FD = [Energy_ch1..4, Peak_ch1..4]   (n_spikes, 8)
...
L_Ratio.m (Schmitzer-Torbert et al. 2005)
  ...
  ② df       = size(FD, 2)                    # = 8 (4 channels × [Energy, Peak])
...
Bug 1 — FD did not match the intended feature space
  WRONG : joint or per-channel PCA on raw waveform samples, 3 PCs/channel,
          no Energy term
  RIGHT : FD = feature_Energy + feature_Peak per channel — Energy (per-channel
          L2 norm) plus Peak (per-channel maximum raw waveform amplitude),
          rather than an arbitrary PC score
  WHY   : Peak is a physically interpretable, deterministic per-channel
          amplitude measure (unlike a PCA component, whose sign/scale depends
          on the spike population passed in). Energy carries overall signal
          power; Peak carries the amplitude of the extremum, and together
          they span both channels' amplitude and energy without relying on
          a data-dependent PCA basis.
...
def build_feature_matrix(waveforms: np.ndarray) -> np.ndarray:
    """Construct the FD matrix from feature_Energy.m + feature_Peak.m,
    one Energy and one Peak value per channel (4 channels × 2 = 8 dims).
    ...
    Returns
    -------
    FD : (n_spikes, 8)  float64 — columns [Energy_ch1..4, Peak_ch1..4].

    feature_Energy.m  (one value per channel)
        energy = sqrt(sum(w², samples)) / sqrt(n_samples)

    feature_Peak.m  (one value per channel)
        peak = max(w_raw, samples)   # raw waveform peak amplitude on that channel
    """
    ...
    for ch in range(n_channels):
        ...
        # ── feature_Peak.m ──
        peak = np.max(w_raw, axis=1)
        peak_blocks.append(peak[:, None])

    return np.hstack(energy_blocks + peak_blocks)   # (n_spikes, 8)
...
def l_ratio(FD: np.ndarray, cluster_mask: np.ndarray) -> float:
    ...
    df      = FD.shape[1]                                # = 8 (4 channels × [Energy, Peak])
...
def _save_excel(rows: list, ntt_path: str, folder: str, n_features: int):
    ...
    notes = [
        ...
        (f'Feature space: Energy + Peak × 4 channels = '
         f'{n_features} dims (peak amplitude per channel instead of WavePC1)', False),

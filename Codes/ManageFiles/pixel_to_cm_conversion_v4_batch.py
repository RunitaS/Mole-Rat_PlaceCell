...
For every .xlsx tracking file found anywhere under ROOT_DIR (including
subfolders), one per file:
...
# ── USER INPUT ──────────────────────────────────────────────────────────────
ROOT_DIR = r'C:/Runita/NMR/analysis/TrackingCorrection/Circle'  # root folder containing subfolders with .xlsx tracking files
...
MAX_SPEED_CM_PER_S = 90  # frame-to-frame speed above this is treated as a jump artifact
...
def prompt_delete_existing_csvs(root_dir):
    """Find pre-existing .csv files under root_dir and, if any are found,
    ask the user whether to delete them before conversion runs (conversion
    writes new .csv outputs, so stale ones from a previous run can otherwise
    linger or get mixed up with fresh results)."""
    csv_paths = [
    ...
    print(f"Found {len(csv_paths)} existing .csv file(s) under {root_dir}:")
    for path in csv_paths:
        print(f"  {path}")

    answer = input("Delete these before converting? [y/N]: ").strip().lower()
    if answer == 'y':
        for path in csv_paths:
            os.remove(path)
            print(f"Deleted: {path}")
    else:
        print("Keeping existing .csv files.")
...
def main():
    prompt_delete_existing_csvs(ROOT_DIR)
    ...
    for folder, _dirnames, filenames in os.walk(ROOT_DIR):
        ...
        xlsx_files = sorted(
            ...
            and not f.endswith(OUTPUT_SUFFIX + '.xlsx')
            and not f.startswith('~$')  # skip Excel lock files for currently-open workbooks
        )
        ...
        for xlsx_path in xlsx_files:
            print(f"[{folder_name}] Converting {os.path.basename(xlsx_path)} "
                  f"(arena_size_cm={arena_size_cm})")
            convert_file(xlsx_path, arena_size_cm)

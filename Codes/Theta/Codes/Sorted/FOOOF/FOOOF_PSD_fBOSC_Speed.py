...
FBOSCPY_DIR = r'Z:\NMR_group_data\Runita\Codes\myCodes\LFP\NpxCodes\NpxLFP'
...
PROCESSED_PSDS_PKL   = os.path.join(OUTPUT_DIR, 'processed_psds.pkl')  # per-file cleaned PSDs (pre-FOOOF)
FOOOF_RESULTS_PKL    = os.path.join(OUTPUT_DIR, 'fooof_results.pkl')   # full FOOOF fit output: {'fooof_results': [...], 'expanded_fooof_df': DataFrame}
...
# %% --------------------------------------------------------------------
# %% NOTE: This part is carried over UNCHANGED from the original notebook.
# %% It does NOT run on the .ncs PSD pipeline in Part 1 — these analyses
# %% read their own separate data (fBOSC episodesTables pickles and
# %% tracking files) and still expect the original FileManager-based
# %% objects (file_manager, day_arena_map, filtered_episodes, ...).
# %% Wire those inputs to your data before answering "y", or answer "n"
# %% to skip. See the note in the accompanying message.
# %% ####################################################################
...
if not _run_fbosc:
    ...
    def plot_episode_properties(episodes_df, pepisode_df=None,
                                 day_arena_map=None, save=False, save_dir=None):
    ...
    day_arena_map = None
    plot_episode_properties(filtered_episodes, pepisode_df=pepisode_df, day_arena_map=day_arena_map, save=False, save_dir='figures/')
    ...
    # ---- cell [86] ----
    plot_duration_frequency_relationship(filtered_episodes, day_arena_map=None,save=False, save_dir=None)
    plot_episode_scatter(filtered_episodes, day_arena_map=day_arena_map, save=False, save_dir=None)
    ...
    if not _run_iei:
        ...
        # ---- cell [91] ----
        plot_theta_continuity(filtered_episodes, day_arena_map=day_arena_map, save=False, save_dir=None)
    ...
    # ---- cell [102] ----
    def load_tracking_for_all_sessions(file_manager, animal, day_session_map,
                                        px_per_cm=None, smooth_method='savgol',
                                        savgol_window=11, savgol_poly=3,
                                        gaussian_sigma_s=0.2):
        """
        Load and compute speed for all sessions using FileManager.

        Parameters
        ----------
        file_manager   : FileManager object (already indexed)
        animal         : str
        ...
        for day, sessions in day_session_map.items():
            for session in sessions:
                ...
                # ── get tracking files via FileManager ────────────────────────────
                try:
                    tracking_files = file_manager.get_files(
                        animal=animal, date=day, session=session, category='tracking')
                except ValueError as e:
                    print(f"  No tracking files for {animal} | {day} | {session}: {e}")
                    continue

                if not tracking_files:
                    print(f"  No tracking files for {animal} | {day} | {session}")
                    continue

                # use first tracking file if multiple exist
                if len(tracking_files) > 1:
                    print(f"  Multiple tracking files for {day}/{session}, "
                          f"using: {os.path.basename(tracking_files[0])}")
    ...
    # ---- cell [103] ----
    fa23bd_date_sessions = file_manager.list_indexed_metadata(animal='FA23BD')
    fa1059_data_ssessions = file_manager.list_indexed_metadata(animal='Fa1059')

    filtered_episodes_fa23bd = filtered_episodes[filtered_episodes['animal'] == 'FA23BD']

    pepisode_df_fa23bd = pepisode_df[pepisode_df['animal'] == 'FA23BD']

    # ── 1. load tracking via FileManager and build interpolators ──────────────────
    tracking_dict, interp_dict = load_tracking_for_all_sessions(
        file_manager   = file_manager,   # your indexed FileManager object
        animal         = 'FA23BD',
        day_session_map= fa23bd_date_sessions,
        px_per_cm      = None,
        smooth_method  = 'savgol',
        savgol_window  = 11,
    )

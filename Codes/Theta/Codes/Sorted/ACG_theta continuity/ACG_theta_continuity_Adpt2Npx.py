...
OUTPUT_DIR = os.path.join(ROOT_DIR, 'ACG_theta_continuity_results_v2')
...
def process_ncs_for_acg(fpath, freq_range=ACG_FREQ_RANGE, freq_resolution=ACG_FREQ_RES,
                        epoch_sec=EPOCH_SEC, fs_acg=FS_ACG):
...
def plot_depth_profile(df, animal, depth_map=None, ax=None):
    """Median +/- IQR normalised peak range vs. channel (or depth, if
    `depth_map` -- a {channel: depth_mm} dict -- is supplied), split by
    movement (style of Dunn et al. Supp. Fig. 6a-b)."""
    summ = summarize_peak_range(df[df['animal'] == animal])
    ax = ax or plt.gca()
    for movement, color in (('moving', '#E67E22'), ('immobile', '#7F8C8D')):
        s = summ[summ['movement'] == movement].sort_values('channel')
        if s.empty:
            continue
        y = s['channel'].map(depth_map) if depth_map else pd.to_numeric(s['channel'], errors='coerce')
        ax.plot(s['median'], y, color=color, marker='o', label=movement)
        ax.fill_betweenx(y, s['q25'], s['q75'], color=color, alpha=0.25)
    if depth_map:
        ax.invert_yaxis()
        ax.set_ylabel('Depth (mm)')
    else:
        ax.set_ylabel('Channel')
    ax.set_xlabel('Autocorr. peak range (norm.)')
    ax.set_title(animal)
    ax.legend()
    return ax
...
def plot_freq_peakrange_summary(df, animal=None, speed_vmax=50.0, moving_cmap='viridis',
    ...
    cbar = fig.colorbar(sc, ax=ax_right, fraction=0.2, pad=0.35)

...
OUTPUT_DIR = r'F:/Temp/Theta/fBOSC_Res/v9_Res0.1/Wavenum12'  # saved plots go here
...
FBOSCPY_DIR = r'F:/Temp/Theta/fBOSC_Res/v9_Res0.1'
...
FBOSC_USE_CACHE    = True   # reuse FBOSC_EPISODES_PKL if present instead of re-running the wavelet transform on every file
                             ...
                             # NOTE: also delete it (or set this False) after pulling in
                             # noise-epoch rejection (see PART 2's _drop_noise_episodes) --
                             # an existing cache was built without it and still includes
                             # noise-epoch episodes. Also make sure PROCESSED_PSDS_PKL was
                             # regenerated first (re-run the FOOOF/PSD checkpoint), since
                             # noise-epoch rejection needs the 'epoch_keep' mask it saves.
...
def compute_psd_clean_epochs(lfp, fs_hz, nperseg, mad_thresh=5.0,
    ...
    Returns
    -------
    f, Pxx, n_clean, n_total, keep
        `keep` is the per-epoch boolean mask (len n_total, True = epoch
        survived all three rejection stages) -- exposed so callers that need
        a continuous trace (e.g. fBOSC, which can't have epochs removed from
        it) can still know which stretches are noise and act on that
        afterwards, instead of only getting the epoch-averaged PSD here.
    """
    ...
    return f, Pxx, n_clean, n_total, keep
...
def process_animal(label, folder):
    ...
    file_names = []
    freq_vec   = None
    lfp_store  = {}   # {rel_file_name: {'lfp':..., 'fs':..., 'start_us':..., 'epoch_keep':...}} -- for fBOSC (Part 2)
    ...
    for fpath in ncs_files:
        ...
        try:
            ...
            f, Pxx, n_clean, n_total, epoch_keep = compute_psd_clean_epochs(
            ...
            # epoch_keep: per-2s-epoch boolean mask (velocity + delta/theta + MAD
            # rejection, same criteria used for the PSD above) over this file's
            # FULL continuous trace -- fBOSC (Part 2) needs the trace intact, so
            # noise epochs aren't removed from `lfp` here, only flagged for it to
            # act on downstream (see PART 2's noise-epoch episode filtering).
            lfp_store[rel] = {'lfp': lfp, 'fs': fs_down, 'start_us': lfp_start_us,
                              'epoch_keep': epoch_keep}
            if speed_keep is not None:
...
if not _run_fbosc:
    ...
    def _clean_sample_mask(epoch_keep, nperseg_, n_samples):
        """Expand a per-epoch keep mask (True = clean, one bool per
        `nperseg_`-sample epoch -- see compute_psd_clean_epochs's `keep`
        return) to a per-sample boolean mask of length `n_samples`. Samples
        past the last full epoch (the trailing len(lfp) % nperseg_ remainder
        compute_psd_clean_epochs never scores) are treated as noise, since no
        rejection criteria were ever evaluated there."""
        mask = np.zeros(n_samples, dtype=bool)
        n_full = min(len(epoch_keep) * nperseg_, n_samples)
        mask[:n_full] = np.repeat(epoch_keep, nperseg_)[:n_full]
        return mask

    def _drop_noise_episodes(episode_table_, clean_mask, fs_hz):
        """Drop rows of `episode_table_` (fBOSC's significant-theta episodes,
        already labeled against the FULL continuous trace, Onset/Offset in
        microseconds) whose [Onset, Offset) span is not ENTIRELY inside clean
        (non-noise) epochs of `clean_mask` -- i.e. an episode is kept only if
        no part of it overlaps a noise epoch (velocity / delta-theta / MAD
        rejected, see process_animal -> lfp_store[...]['epoch_keep']).

        fBOSC has to run on the full continuous trace (the wavelet transform
        needs an unbroken signal), so noise epochs can't be excised before
        detection -- this removes noise-overlapping episodes AFTER fBOSC has
        already labeled them, so every downstream analysis (p-episode / IEI /
        speed) only sees episodes drawn from clean epochs.
        """
        n_samples = len(clean_mask)
        onset_i  = np.clip(np.round(episode_table_['Onset'].to_numpy()
                                    / 1e6 * fs_hz).astype(int), 0, n_samples)
        offset_i = np.clip(np.round(episode_table_['Offset'].to_numpy()
                                    / 1e6 * fs_hz).astype(int), 0, n_samples)
        is_clean = np.array([(i1 > i0) and clean_mask[i0:i1].all()
                             for i0, i1 in zip(onset_i, offset_i)])
        return episode_table_[is_clean].reset_index(drop=True)
        ...
        for animal, animal_data in processed_psds.items():
            ...
            for rel, file_data in lfp_store.items():
                ...
                try:
                    ...
                    # ---- Noise-epoch rejection, applied AFTER fBOSC's
                    # continuous-signal theta/non-theta labeling above ----
                    # `lfp` still contains the epochs compute_psd_clean_epochs
                    # would normally drop (velocity / delta-theta / MAD) --
                    # fBOSC needed the unbroken trace for its wavelet
                    # transform. Now that episodes are labeled, drop any
                    # episode overlapping a noise epoch before it reaches
                    # p-episode / IEI / speed analyses further down.
                    epoch_keep = file_data.get('epoch_keep')
                    if epoch_keep is None:
                        print(f"    WARNING: {rel} has no cached epoch_keep "
                              f"mask (stale {PROCESSED_PSDS_PKL} from before "
                              "noise-epoch rejection was added) -- re-run the "
                              "FOOOF/PSD checkpoint to regenerate it. "
                              "Noise-epoch rejection skipped for this file.")
                    else:
                        clean_mask = _clean_sample_mask(epoch_keep, nperseg, len(lfp))
                        n_pre_reject = len(episode_table)
                        episode_table = _drop_noise_episodes(episode_table, clean_mask, fs_file)
                        n_dropped = n_pre_reject - len(episode_table)
                        if n_dropped:
                            print(f"    Noise-epoch rejection: dropped "
                                  f"{n_dropped}/{n_pre_reject} episodes "
                                  "overlapping a noise epoch")
                        if episode_table.empty:
                            print(f"  SKIP: {rel} -- 0 episodes remain after "
                                  "noise-epoch rejection")
                            continue

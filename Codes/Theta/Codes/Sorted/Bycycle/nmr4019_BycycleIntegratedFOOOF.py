...
OUTPUT_DIR = r'F:/Temp/Theta/Bycycle_Res/Bycycle_FOOOF_Integ_v2'  # saved plots go here
...
BYCYCLE_FILTER_SECONDS = 0.5 #0.5          # s; lowpass filter length. Decreasing this increases number of theta cycles.
...
def plot_fooof_fits_for_files(processed_psds, file_pairs, fooof_settings=None,
                              fooof_range=None, theta_band=None, save_dir=None,
                              fit_xlim=(1, 20)):
    """Fit + plot FOOOF (original spectrum, full model, aperiodic fit, theta
    range) for a specific set of (animal, file) pairs, using the SAME
    FOOOF_SETTINGS/FOOOF_RANGE as PART 1's build_fooof_results() -- so these
    fits are directly comparable to Part 1's, just refit one file at a time
    (plain FOOOF, not FOOOFGroup) for whichever files are passed in.

    Intended use: after compute_bycycle_features(), pass the unique
    (animal, file) pairs that actually appear in df_bycycle, so every .ncs
    file bycycle characterized cycles from also gets its FOOOF fit plotted --
    letting you check, file by file, whether a low bursting-cycle count
    reflects a genuinely weak/absent theta peak (visible here) or a bycycle
    threshold/preprocessing issue on top of a real oscillation.

    Parameters
    ----------
    processed_psds : dict  loaded from PROCESSED_PSDS_PKL --
                     {animal: {'freqs':, 'psds':, 'files':, ...}}
    file_pairs      : iterable of (animal, file) tuples to plot -- `file` must
                      match an entry in that animal's 'files' list (i.e. the
                      same relative path used throughout this script)
    save_dir        : default BYCYCLE_OUTPUT_DIR/fooof_fits
    ...
    fooof_range    = fooof_range or FOOOF_RANGE
    theta_band     = theta_band or THETA_BAND
    save_dir       = save_dir or os.path.join(BYCYCLE_OUTPUT_DIR, 'fooof_fits')

    n_plotted = 0
    n_skipped = 0
    for animal, rel in file_pairs:
        animal_data = processed_psds.get(animal)
        if animal_data is None:
            print(f"  SKIP FOOOF fit: {animal}/{rel} -- animal not in processed_psds")
            n_skipped += 1
            continue
        files = list(animal_data['files'])
        try:
            idx = files.index(rel)
        except ValueError:
            print(f"  SKIP FOOOF fit: {animal}/{rel} -- file not in processed_psds['files']")
            n_skipped += 1
            continue

        freqs = animal_data['freqs']
        psd   = animal_data['psds'][idx]
    ...
    print(f"Saved {n_plotted} FOOOF fit figures for bycycle-analyzed files -> "
         f"{save_dir} ({n_skipped} skipped)")
    return n_plotted
...
if not _run_bycycle:
    ...
    # FOOOF fit plot (same FOOOF_SETTINGS/FOOOF_RANGE as PART 1) for every
    # .ncs file that actually contributed cycles to df_bycycle -- lets you
    # check, file by file, whether a low bursting-cycle count reflects a
    # genuinely weak/absent theta peak or a bycycle-side issue.
    _bycycle_file_pairs = df_bycycle[['animal', 'file']].drop_duplicates().itertuples(
        index=False, name=None)
    plot_fooof_fits_for_files(processed_psds, _bycycle_file_pairs)

...
Output_PlaceTrue = r'C:/Runita/NMR/analysis/AllSort_Results/PlaceCell/Data/PlaceCell_True_v2/Fa1059/AdptBin'
fps            = 30           # tracking frame rate (Hz)
target_bin_cm  = 2.0          # bin size in cm
...
ADAPTIVE_ALPHA_BASE = 0.0001  # adaptive-binning smoothing constant (Jim uses 0.0001; Skaggs
                               # C code uses 0.001), scaled by fps below since occ_map is in
                               # seconds rather than sample counts
ADAPTIVE_MAX_RSQ     = 200    # max squared search radius (bin units) before an adaptively
...
def _adaptive_binned_ratemap(occ_map: np.ndarray, spike_map: np.ndarray,
                              min_occ_s: float, fps: float,
                              alpha_base: float = ADAPTIVE_ALPHA_BASE,
                              max_rsq: int = ADAPTIVE_MAX_RSQ) -> tuple[np.ndarray, np.ndarray]:
    """Adaptive binning firing rate map (Yartsev & Ulanovsky 2013; Grieves 2023
    review, "Adaptive binning"), replacing the fixed-bin histogram + triangular
    smoothing. Ported from the lab's getAdaptiveBinnedRmap.m.

    For every occupied bin, a circular neighbourhood (radius in bin units,
    i.e. squared radius rsq = 0, 1, 2, ...) is grown outward until either
        alpha^2 * n_occ^2 * rsq * n_spikes2 > 1
    (the adaptive-smoothing criterion, Skaggs & McNaughton 1998 eq. 11) or
    rsq reaches max_rsq. n_spikes2 is the spike count already enclosed at
    that radius, or 1 if none has fallen within it yet (avoids a zero
    threshold, matching the MATLAB reference). The bin's rate is then
    spikes/occupancy within the final radius; bins whose final enclosed
    occupancy still falls short of min_occ_s -- or that were never visited
    at all -- are left invalid, so sparsely-sampled bins borrow from their
    surroundings instead of being dropped or blurred with a fixed kernel.

    Vectorized per occupied bin: occupancy/spike counts accumulate
    monotonically with radius, so instead of growing rsq one step at a time
    (as in the MATLAB reference), all candidate squared radii 0..max_rsq-1
    are evaluated at once via a distance sort + cumulative sum.
    """
    n_i, n_j = occ_map.shape
    alpha = alpha_base * fps
    ...
    occ_flat   = occ_map.ravel()
    spike_flat = spike_map.ravel()

    I, J = np.meshgrid(np.arange(n_i), np.arange(n_j), indexing='ij')
    I = I.ravel(); J = J.ravel()

    rsq_values = np.arange(max_rsq, dtype=np.float64)

    for i0, j0 in np.argwhere(occ_map > 0):
        d2 = (I - i0) ** 2 + (J - j0) ** 2
        ...
        # idx(rsq) = number of bins enclosed within squared radius rsq
        idx = np.searchsorted(d2_sorted, rsq_values, side='right')
        idx = np.clip(idx, 1, len(d2_sorted))   # rsq=0 already encloses the source bin itself
        occ_at_rsq   = occ_cum[idx - 1]
        spike_at_rsq = spike_cum[idx - 1]

        nspikes2  = np.where(spike_at_rsq > 0, spike_at_rsq, 1.0)
        satisfied = (alpha * alpha) * (occ_at_rsq ** 2) * rsq_values * nspikes2 > 1.0

        k = int(np.argmax(satisfied)) if satisfied.any() else max_rsq - 1

        n_occ_final    = occ_at_rsq[k]
...
def _compute_split_half_stability(ctx: dict,
    ...
    fr_first_map,  valid_first  = _adaptive_binned_ratemap(occ_first,  spike_first,  min_occ_s, fps)
    fr_second_map, valid_second = _adaptive_binned_ratemap(occ_second, spike_second, min_occ_s, fps)
...
def compute_metrics(csv_path: str, ntt_path: str,
    ...
    # Replaces the fixed-bin histogram + triangular smoothing: each occupied
    # bin's neighbourhood radius grows until it encloses enough occupancy and
    # spikes (Yartsev & Ulanovsky 2013; Grieves 2023 review, "Adaptive
    # binning"), so sparsely sampled bins borrow from their surroundings
    # instead of being smoothed with a fixed kernel or dropped outright.
    fr_adaptive, valid_mask_adaptive = _adaptive_binned_ratemap(
        occ_map, spike_map, min_occ_s, fps)

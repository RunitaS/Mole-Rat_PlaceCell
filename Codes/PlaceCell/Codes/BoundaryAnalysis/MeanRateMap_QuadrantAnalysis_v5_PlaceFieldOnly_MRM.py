...
def process_unit(csv_path: str, ntt_path: str, ntt_file: str, session_name: str, handler) -> dict | None:
    ...
    return dict(
        ...
        fr_raw=cell['fr_raw'], fi_map=cell['fi_map'], valid=cell['valid'], occ_map=cell['occ_map'],
...
def pool_field_only_map(handler, results: list) -> tuple:
    ...
    Pooling is occupancy-weighted per bin (weight = seconds of occupancy, proportional to
    the number of position samples collected there) rather than an unweighted mean across
    cells, so a bin's pooled value is not biased by cells that happened to oversample (or
    undersample) that bin relative to others."""
    place = [r for r in results if r['place_cell']]
    if not place:
        return np.full(handler.n_bins, np.nan), np.zeros(handler.n_bins, dtype=bool)
...
def plot_field_only_mean_maps(arena_handlers: dict, arena_results: dict, save_path: str):
    ...
    oversampled relative to others."""
    fig = plt.figure(figsize=(15, 5))
    for i, key in enumerate(_ARENA_ORDER):
        handler = arena_handlers[key]

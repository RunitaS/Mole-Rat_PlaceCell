...
# ============================================================================
# Simulated-null KDE analysis (tiled Gaussian-field null, run through the real pipeline)
#
#   1. KDE fit of the observed maps. Each of the three whole-arena pooled maps
#        'overall' : pool_fine_map              (overall mean field-index map)
#        'peak'    : pool_peak_proportion_fine  (% of place-cell peak bins per bin)
#        'field'   : pool_field_only_map        (place-field-only mean map)
#      is fit with a Gaussian kernel of NULLSIM_KDE_SIGMA_BINS over the arena's sampled bins.
#      For the peak map (a histogram of points) this is a 2D kernel density estimate of peak
#      locations; for the two mean maps it is the kernel-weighted (Nadaraya-Watson) mean,
#      i.e. _gaussian_smooth_2d's normalised convolution, so walls and unsampled bins do not
#      drag the fit toward 0.
#   2. Observed null. The arena is tiled with non-overlapping NULLSIM_TILE_BINS (3 x 3) place
#      fields. Each field fires with a Gaussian profile (NULLSIM_FIELD_SIGMA_BINS) peaking at
#      its central bin, above a baseline rate outside the field. Tiles are clipped at the arena
#      boundary (the 2-bin-wide circular track gets 3 x 2 fields; the open field's circular wall
#      cuts its edge tiles), and every one of the tx*ty offsets of the tiling lattice is used, so
#      no lattice alignment is favoured. Every real place cell gets one synthetic twin: same
#      session (so the same per-bin occupancy from that session's tracking and the same valid
#      bins), same peak and baseline rate, but with its field drawn uniformly from the tiles
#      that session sampled. Its spike count in each bin is Poisson(rate x dwell time) -- what
#      the animal would record passing through that field map -- and the twin then goes through
#      the real per-cell pipeline (raw rate -> smoothing -> field index -> peak bin -> field
#      mask) and the same pooling as the real cells. This is repeated NULLSIM_N_SIM times; the
#      mean over realisations is the stable observed null.
#   3. Both KDE fits (observed, and every null realisation) are smoothed a second time
#      (NULLSIM_POST_SMOOTH_SIGMA_BINS), then rescaled to % of map total (NULLSIM_NORMALIZE),
#      so the comparison is about WHERE the map's mass lies, not its overall level (a real
#      field is larger than a 3 x 3 tile, which alone shifts every bin's level).
#   4. Per-bin Monte Carlo test. The NULLSIM_N_SIM null maps are the distribution the observed
#      map would come from if fields tiled the arena uniformly, with the same cell count and
#      the same occupancy, so p = two-sided rank of the observed value among them. The signed
#      significance, sign(observed - null mean) * -log10(p), is the heat map; bins passing
#      Benjamini-Hochberg FDR (NULLSIM_FDR_Q) across the tested bins are outlined. Its floor is
#      p = 2 / (NULLSIM_N_SIM + 1), so NULLSIM_N_SIM sets how strong a bin can ever look.
# ============================================================================

NULLSIM_TILE_BINS              = (3, 3)   # place-field tile, (bins along x/arc/length, bins along y/radial/width)
NULLSIM_FIELD_SIGMA_BINS       = 1.0      # Gaussian fall-off of the rate from the tile's central bin
NULLSIM_KDE_SIGMA_BINS         = RATEMAP_SMOOTH_SIGMA_BINS   # kernel bandwidth of the KDE fit (step 1)
NULLSIM_POST_SMOOTH_SIGMA_BINS = 1.5      # second smoothing of both KDE fits (step 3)
NULLSIM_N_SIM                  = 1000     # null realisations (sets the smallest attainable p)
NULLSIM_NORMALIZE              = 'pct'    # 'pct' (% of map total) or 'none' (native map units)
# 'tile'  : a synthetic cell's place field is its tile (restricted to the session's valid bins)
# 'detect': detect it from the simulated rate map with extract_place_field_mask, like a real
#           cell -- note a 3 x 3 tile can never reach MIN_FIELD_BINS on the 2-bin-wide circular
#           track, so 'detect' leaves that arena's field-only null empty
NULLSIM_FIELD_MASK             = 'tile'
NULLSIM_FDR_Q                  = 0.05


def _smooth_sigma(handler, values_flat: np.ndarray, valid_flat: np.ndarray,
                  sigma: float) -> np.ndarray:
    """handler.smooth with an explicit kernel width (handler.smooth is fixed at
    RATEMAP_SMOOTH_SIGMA_BINS); wraps the circular track's arc-length axis like it does."""
    wrap = isinstance(handler, CircularTrackHandler)
    return _gaussian_smooth_2d(values_flat.reshape(handler.nx, handler.ny),
                               valid_flat.reshape(handler.nx, handler.ny),
                               sigma=sigma, wrap_x=wrap).ravel()


def _smooth_batch(handler, fr: np.ndarray, valid: np.ndarray, sigma: float) -> np.ndarray:
    """_gaussian_smooth_2d applied to a stack of flat maps (n, n_bins), each with its own
    valid mask, in one gaussian_filter call (sigma 0 on the stack axis)."""
    n = fr.shape[0]
    wrap = isinstance(handler, CircularTrackHandler)
    mode = ('constant', 'wrap' if wrap else 'constant', 'constant')
    f3 = np.where(valid, fr, 0.0).reshape(n, handler.nx, handler.ny)
    w3 = valid.astype(np.float64).reshape(n, handler.nx, handler.ny)
    sf = gaussian_filter(f3, sigma=(0, sigma, sigma), mode=mode, cval=0.0).reshape(n, -1)
    sw = gaussian_filter(w3, sigma=(0, sigma, sigma), mode=mode, cval=0.0).reshape(n, -1)
...
def _nullsim_tiles(handler) -> tuple:
    """Every tile of every offset of the non-overlapping NULLSIM_TILE_BINS tiling. Returns
    (gain, members): gain (n_tiles, n_bins) is the Gaussian rate profile (1 at the tile's
    central bin, 0 outside the tile), members the tile's in-arena bins. A clipped edge tile
    keeps its nominal centre, so it carries only the tail of its field."""
    nx, ny = handler.nx, handler.ny
    tx, ty = NULLSIM_TILE_BINS
    wrap = isinstance(handler, CircularTrackHandler) and nx % tx == 0
    geom = getattr(handler, 'geom_valid', np.ones(handler.n_bins, dtype=bool))
    s2 = 2.0 * NULLSIM_FIELD_SIGMA_BINS ** 2
    gains = []
    for ox in range(tx):
        for oy in range(ty):
            x0s = range(ox, nx, tx) if wrap else range(ox - tx, nx, tx)
            for x0 in x0s:
                for y0 in range(oy - ty, ny, ty):
                    cx, cy = x0 + (tx - 1) / 2.0, y0 + (ty - 1) / 2.0
                    g = np.zeros(handler.n_bins)
                    for x in range(x0, x0 + tx):
                        if not wrap and not 0 <= x < nx:
                            continue
                        for y in range(max(y0, 0), min(y0 + ty, ny)):
                            flat = (x % nx) * ny + y
                            if geom[flat]:
                                g[flat] = np.exp(-((x - cx) ** 2 + (y - cy) ** 2) / s2)
                    if g.any():
                        gains.append(g)
    gain = np.array(gains)
    return gain, gain > 0
...
def _nullsim_twins(cells: list, members: np.ndarray) -> dict:
    """Per-cell inputs of the synthetic twins: the real cell's session occupancy and valid
    bins, its peak (smoothed) rate and baseline (lowest smoothed rate), and the tiles its
    session sampled at least one bin of. Cells whose session samples no tile are dropped."""
    valid = np.array([r['valid'] for r in cells])
    seen = (members.astype(np.float64) @ valid.T.astype(np.float64)) > 0   # (n_tiles, n_cells)
    keep = [i for i in range(len(cells)) if seen[:, i].any()]
    return dict(
        session=[cells[i]['session'] for i in keep],
        valid=valid[keep],
        occ=np.array([np.where(cells[i]['valid'], cells[i]['occ_map'], 0.0) for i in keep]),
        peak=np.array([float(cells[i]['peak_fr']) for i in keep]),
        base=np.array([float(cells[i]['fr_smooth'][cells[i]['valid']].min()) for i in keep]),
        eligible=[np.flatnonzero(seen[:, i]) for i in keep],
    )


def _nullsim_population(handler, twins: dict, gain: np.ndarray, members: np.ndarray,
                        rng: np.random.Generator) -> list:
    """One null realisation: a synthetic twin per real cell, with a random tile as its field,
    Poisson spikes from its session's occupancy, and the per-cell outputs of
    compute_cell_ratemap / process_unit (fi_map, peak_bin, field_mask) the pooling uses."""
    tile = np.array([e[rng.integers(len(e))] for e in twins['eligible']])
    valid, occ, base = twins['valid'], twins['occ'], twins['base']
    rate = base[:, None] + (twins['peak'] - base)[:, None] * gain[tile]
    spikes = rng.poisson(np.where(valid, rate * occ, 0.0))
    fr_raw = np.zeros_like(occ)
    np.divide(spikes, occ, out=fr_raw, where=valid)
    fr_s = _smooth_batch(handler, fr_raw, valid, RATEMAP_SMOOTH_SIGMA_BINS)

    vmin = np.where(valid, fr_s, np.inf).min(axis=1)
    vmax = np.where(valid, fr_s, -np.inf).max(axis=1)
    span = vmax - vmin
    fi = np.where(valid, (fr_s - vmin[:, None]) / np.where(span > 0, span, 1.0)[:, None], np.nan)
    peak_bin = np.argmax(np.where(valid, fr_s, -np.inf), axis=1)

    if NULLSIM_FIELD_MASK == 'tile':
        fmask = members[tile] & valid
    else:
        pi = occ / occ.sum(axis=1, keepdims=True)
        mean_fr = (pi * fr_s).sum(axis=1)
        fmask = np.array([extract_place_field_mask(dict(valid=valid[i], fr_raw=fr_raw[i],
                                                        mean_fr=mean_fr[i]), handler)
                          for i in range(len(tile))])

    return [dict(session=twins['session'][i], fi_map=fi[i], valid=valid[i], occ_map=occ[i],
                 field_mask=fmask[i], peak_bin=int(peak_bin[i]) if vmax[i] > 0 else None)
            for i in range(len(tile))]


def _nullsim_raw_maps(handler, cells: list) -> tuple:
    """The three pooled whole-arena maps of one population (real or synthetic), 0-filled
    inside the sampled bins -- a sampled bin no field covers is a genuine zero of the
    field-only / peak maps, not missing data. Returns (maps, sampled)."""
    sampled = np.zeros(handler.n_bins, dtype=bool)
    for r in cells:
        sampled |= r['valid']
    overall, _ = pool_fine_map(handler, cells)
    field, _   = pool_field_only_map(handler, cells)
    peak, _    = pool_peak_proportion_fine(handler, cells)
    maps = {k: np.where(sampled & np.isfinite(v), v, 0.0)
            for k, v in (('overall', overall), ('peak', peak), ('field', field))}
    return maps, sampled


def _nullsim_fit(handler, values: np.ndarray, sampled: np.ndarray) -> np.ndarray:
    """KDE fit (step 1), second smoothing (step 3), then NULLSIM_NORMALIZE."""
    kde = _smooth_sigma(handler, values, sampled, NULLSIM_KDE_SIGMA_BINS)
    fit = _smooth_sigma(handler, kde, sampled, NULLSIM_POST_SMOOTH_SIGMA_BINS)
    return _normalize_pct(fit) if NULLSIM_NORMALIZE == 'pct' else fit


def _bh_reject(p: np.ndarray, q: float) -> np.ndarray:
    """Benjamini-Hochberg: True where p is rejected at FDR q."""
    m = len(p)
...
def _nullsim_test(handler, obs: np.ndarray, null_stack: np.ndarray, tested: np.ndarray) -> dict:
    """Per-bin two-sided Monte Carlo test of the observed fit against the null realisations
    (step 4), BH-FDR over the tested bins, and clusters of contiguous significant bins."""
    n_sim = null_stack.shape[0]
    tol = 1e-12 * max(1.0, float(np.abs(obs).max()))
    n_ge = (null_stack >= obs[None, :] - tol).sum(axis=0)
    n_le = (null_stack <= obs[None, :] + tol).sum(axis=0)
    p = np.minimum(1.0, 2.0 * (np.minimum(n_ge, n_le) + 1) / (n_sim + 1))
    null_mean = null_stack.mean(axis=0)
    null_sd = null_stack.std(axis=0, ddof=1) if n_sim > 1 else np.zeros_like(null_mean)
    diff = obs - null_mean
    z = np.where(null_sd > 0, diff / np.where(null_sd > 0, null_sd, 1.0), 0.0)
    signed = np.sign(diff) * -np.log10(p)

    fdr = np.zeros(len(obs), dtype=bool)
    fdr[tested] = _bh_reject(p[tested], NULLSIM_FDR_Q)
    clusters = []
    for direction, mask in (('enriched', fdr & (diff > 0)), ('depleted', fdr & (diff < 0))):
        for region in handler.connected_components(mask):
            top = int(region[np.argmax(np.abs(signed[region]))])
            clusters.append(dict(direction=direction, n_bins=len(region), peak_bin=top,
                                 obs=float(obs[top]), null_mean=float(null_mean[top]),
                                 z=float(z[top]), p=float(p[top])))
    return dict(obs=obs, null_mean=null_mean, null_sd=null_sd, diff=diff, z=z, p=p,
                signed_log10p=signed, fdr_mask=fdr, tested=tested, clusters=clusters,
                n_sim=n_sim)


def analyze_null_sim_kde(handler, cells: list, n_sim: int = NULLSIM_N_SIM,
                         rng: np.random.Generator | None = None) -> dict:
    """Simulated-null KDE analysis for one arena (see the section comment above). Returns
    {map_type: result of _nullsim_test, plus n_place_cells / n_null_cells / n_tiles}."""
    rng = rng if rng is not None else np.random.default_rng(0)
    n_b = handler.n_bins
    if not cells:
        blank = _nullsim_test(handler, np.zeros(n_b), np.zeros((2, n_b)), np.zeros(n_b, dtype=bool))
        return {m: dict(blank, n_place_cells=0, n_null_cells=0, n_tiles=0) for m in _MAP_ORDER}

    obs_raw, sampled = _nullsim_raw_maps(handler, cells)
    obs = {m: _nullsim_fit(handler, obs_raw[m], sampled) for m in _MAP_ORDER}

    gain, members = _nullsim_tiles(handler)
    twins = _nullsim_twins(cells, members)
    null = {m: np.zeros((n_sim, n_b)) for m in _MAP_ORDER}
    for s in range(n_sim):
        raw, samp_s = _nullsim_raw_maps(handler, _nullsim_population(handler, twins, gain, members, rng))
        for m in _MAP_ORDER:
            null[m][s] = _nullsim_fit(handler, raw[m], samp_s)

    return {m: dict(_nullsim_test(handler, obs[m], null[m], sampled),
                    n_place_cells=len(cells), n_null_cells=len(twins['session']),
                    n_tiles=len(gain))
            for m in _MAP_ORDER}


def plot_null_sim_kde(arena_handlers: dict, results: dict, map_type: str, save_path: str):
    """One figure per map type, one column per arena; rows = observed KDE fit, stable null
    (mean of the null fits, same colour scale as the observed row), observed - null, and the
    signed significance heat map. Outline = bins significant after BH-FDR."""
    unit = '% of map total' if NULLSIM_NORMALIZE == 'pct' else 'map units'
    fig = plt.figure(figsize=(15, 15))
    gs = fig.add_gridspec(4, 3, hspace=0.5, wspace=0.3)
    sig_cmap = _get_cmap('RdBu_r')
    sig_cmap.set_bad('white')
    ...
    for ci, arena_key in enumerate(_ARENA_ORDER):
        ...
        tv = res['tested']
        proj = 'polar' if arena_key == 'circular_track' else None
        main = make_cmap_norm(np.concatenate([res['obs'][tv], res['null_mean'][tv]]))
        cap = -np.log10(2.0 / (res['n_sim'] + 1))
        rows = [
            (res['obs'],           main,                                 unit,
             f"{_ARENA_TITLES[arena_key]}\nobserved KDE fit (n={res['n_place_cells']} cells)"),
            (res['null_mean'],     main,                                 unit,
             f"tiled-field null ({res['n_sim']} sims, {res['n_tiles']} tiles)"),
            (res['diff'],          _diverging_cmap_norm(res['diff'][tv]), 'obs - null',
             'observed - null'),
            (res['signed_log10p'], (sig_cmap, Normalize(-cap, cap)),     'sign x -log10(p)',
             f"signed significance ({len(res['clusters'])} FDR cluster"
             f"{'s' if len(res['clusters']) != 1 else ''})"),
        ]
        for ri, (vals, (cmap, norm), label, title) in enumerate(rows):
            ax = fig.add_subplot(gs[ri, ci], projection=proj)
            im = handler.plot_fine(ax, np.nan_to_num(vals), tv, cmap, norm)
            if ri >= 2:
                handler.overlay_fine_significance(ax, res['fdr_mask'])
            ax.set_title(title, fontsize=8)
            fig.colorbar(im, ax=ax, fraction=0.07, aspect=18, pad=0.06, label=label)

    fig.suptitle(f'{_MAP_TITLES[map_type]}: KDE fit vs. simulated tiled-field null '
                 f'({NULLSIM_TILE_BINS[0]}x{NULLSIM_TILE_BINS[1]}-bin Gaussian fields, session occupancy, '
                 f'Poisson spiking)\nkernel sigma {NULLSIM_KDE_SIGMA_BINS:g} bins + second smoothing '
                 f'{NULLSIM_POST_SMOOTH_SIGMA_BINS:g} bins; red = above null, blue = below; '
                 f'outline = BH-FDR q < {NULLSIM_FDR_Q:g}')
    fig.savefig(save_path, dpi=200)
    plt.close(fig)
    print(f'[SAVED] {save_path}')


def export_null_sim_kde(arena_handlers: dict, results: dict, out_path: str):
    bin_rows, cl_rows = [], []
    for arena_key, by_map in results.items():
        ny = arena_handlers[arena_key].ny
        for map_type, res in by_map.items():
            for b in np.flatnonzero(res['tested']):
                bx, by = divmod(int(b), ny)
                bin_rows.append(dict(arena=arena_key, map_type=map_type, bin=int(b), bx=bx, by=by,
                                     observed=res['obs'][b], null_mean=res['null_mean'][b],
                                     null_sd=res['null_sd'][b], obs_minus_null=res['diff'][b],
                                     z=res['z'][b], p=res['p'][b],
                                     signed_log10p=res['signed_log10p'][b],
                                     fdr_sig=bool(res['fdr_mask'][b])))
            for i, cl in enumerate(res['clusters'], start=1):
                bx, by = divmod(cl['peak_bin'], ny)
                cl_rows.append(dict(arena=arena_key, map_type=map_type,
                                    n_place_cells=res['n_place_cells'], cluster=i,
                                    peak_bx=bx, peak_by=by,
                                    **{k: v for k, v in cl.items() if k != 'peak_bin'}))
    with pd.ExcelWriter(out_path) as xw:
        pd.DataFrame(bin_rows).to_excel(xw, sheet_name='Bins', index=False)
        pd.DataFrame(cl_rows).to_excel(xw, sheet_name='Clusters', index=False)
    print(f'[SAVED] {out_path}')


def run_null_sim_kde_analysis(arena_handlers: dict, arena_results: dict, out_dir: str,
                              n_sim: int = NULLSIM_N_SIM) -> dict:
    results = {}
    for arena_key in _ARENA_ORDER:
        rng = np.random.default_rng(_stable_seed('nullsim', arena_key))
        results[arena_key] = analyze_null_sim_kde(arena_handlers[arena_key],
                                                  arena_results[arena_key], n_sim=n_sim, rng=rng)
        for map_type in _MAP_ORDER:
            res = results[arena_key][map_type]
            n_enr = sum(c['direction'] == 'enriched' for c in res['clusters'])
            print(f'[{arena_key}/{map_type} null-sim KDE] n={res["n_place_cells"]} place cells, '
                  f'{int(res["fdr_mask"].sum())} FDR-significant bins '
                  f'({n_enr} enriched / {len(res["clusters"]) - n_enr} depleted clusters)')

    for map_type in _MAP_ORDER:
        plot_null_sim_kde(arena_handlers, results, map_type,
                          os.path.join(out_dir, f'NullSim_KDE_{map_type}.png'))
    export_null_sim_kde(arena_handlers, results, os.path.join(out_dir, 'NullSim_KDE_Significance.xlsx'))
    return results
...
def run_full_pipeline(out_dir: str) -> None:
    ...
    run_null_sim_kde_analysis(arena_handlers, arena_results, out_dir)

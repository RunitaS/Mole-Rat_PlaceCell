...
# PlaceCellCharacterization_SI_Spar_Cohr_PeakFR_MeanFR_*), split by each cell's
# zone assignment (classify_edge_centre) and compared with a two-sided
# Mann-Whitney U test — matches the nonparametric test already used for
# dist_from_wall_cm in plot_zone_boxplot below.
...
def _zone_metric_comparison(pc_full: pd.DataFrame) -> pd.DataFrame:
    """Mann-Whitney U comparison of place-cell metrics between edge and centre zone cells."""
    rows = []
    for col, label in ZONE_METRICS:
        ...
        if len(edge_vals) >= 2 and len(centre_vals) >= 2:
            ...
            u_stat, p_val, sig = float('nan'), float('nan'), 'Insufficient data'
...
def run_boundary_analysis(cfg: ArenaConfig) -> str | None:
    ...
    print(f'\n── Edge vs Centre metric comparison (place cells, Mann-Whitney U) ──')
    for _, row in df_zone_metrics.iterrows():
        print(f'{row["Metric"]:38s}  edge={row["Median_edge"]:>8}  centre={row["Median_centre"]:>8}  '
              f'U={row["Mann_Whitney_U"]:>10}  p={row["p_value"]:>8}  sig={row["Significant_p<0.05"]}')
...
def load_metric_zone_data(excel_file: str, metric_col: str):
    """Load a metric column and zone from an Excel file's 'Full' sheet, filtered to place cells."""

...
ROOT_DIRECTORY = r'X:\NMR_group_data\Runita\Analysis\Thesis\Data_v2_Accepted\All_TT_PlaceTrue'
...
OUTPUT_DIR = r'X:\NMR_group_data\Runita\Analysis\Thesis\Data_v2_Accepted\MeanRM_Quad'
...
def analyze_quadrant_kde(handler, results: list, map_type: str,
    ...
    out = dict(map_type=map_type, quad_shape=(qx, qy), n_place_cells=len(place_cells),
               ...
               sig_mask=np.zeros(n_quad, dtype=bool),
               quad_valid=np.zeros(n_quad, dtype=bool), clusters=[])
    if not place_cells:
        return out
    ...
    sig = (out['real_lo'] > out['null_kde']) & out['quad_valid']
    out['sig_mask'] = sig
    out['clusters'] = [
...
def _build_fine_null_kde(handler, cells: list, map_type: str) -> np.ndarray:
    """Occupancy-null counterpart of _build_fine_observed_kde: KDE-smooths the pooled
    dwell-time map (field-restricted for 'field', unrestricted for 'overall'/'peak') over the
    same whole-arena grid, and rescales it to % of its own total."""
...
def analyze_fine_kde(handler, results: list, map_type: str,
    ...
    out = dict(map_type=map_type, n_place_cells=len(place_cells),
               real_med=np.full(n_fine, np.nan), real_lo=np.full(n_fine, np.nan),
               real_hi=np.full(n_fine, np.nan), null_kde=np.full(n_fine, np.nan),
               sig_mask=np.zeros(n_fine, dtype=bool),
               fine_valid=np.zeros(n_fine, dtype=bool), clusters=[])
    if not place_cells:
        return out

    out['null_kde']   = _build_fine_null_kde(handler, place_cells, map_type)
    ...
    sig = (out['real_lo'] > out['null_kde']) & out['fine_valid']
    out['sig_mask'] = sig
    out['clusters'] = [
        dict(n_bins=len(region),
             peak_local_idx=int(region[np.argmax(out['real_med'][region])]),
...
def run_full_pipeline(out_dir: str) -> None:
    ...
    run_fine_kde_analysis(arena_handlers, arena_results, out_dir)

...
COVERAGE_FRACTION = 0.50      # lowered from 0.80 (via an intermediate 0.75): a 75% run
                               # showed circular_track sessions only reach 52-60% coverage and
                               # open_field sessions only 29-35% (0 units passed either arena at
                               # 75%; see Debug_MeanRateMap_QuadrantAnalysis_v8_CirBinLinCorrection.py
                               # for the per-unit raw-bin check that confirmed circular_track's bin
                               # assignment itself was fine) -- a session must have >= min_occ_s
                               # occupancy in at least this fraction of the arena's total spatial
                               # bins (per handler, see `total_arena_bins`/`coverage_threshold_bins`),
                               # else every file (unit) from that session is skipped -- computed
                               # dynamically per handler rather than hardcoded, so it tracks
                               # target_bin_cm/geometry
...
def pool_field_only_map(handler, results: list) -> tuple:
    """Mean (raw Hz) rate map pooled ONLY over each place cell's own extracted place-field
    ...
    Unlike pool_fine_map, cells are NOT normalized to their own 0-1 field index first:
    per Mean_RM_Fixes.md, normalizing here would undo the very boundary effect (peak-rate
    differences across the arena) this map is meant to reveal. Pooling is occupancy-weighted
    per bin (weight = seconds of occupancy, proportional to the number of position samples
    collected there) rather than an unweighted mean across cells, so a bin's pooled value is
    not biased by cells that happened to oversample (or undersample) that bin relative to
    others."""
    place = [r for r in results if r['place_cell']]
    if not place:
        return np.full(handler.n_bins, np.nan), np.zeros(handler.n_bins, dtype=bool)
...
def plot_field_only_mean_maps(arena_handlers: dict, arena_results: dict, save_path: str):
    """Additional mean rate maps built ONLY from each place cell's extracted place-field
    bins (pool_field_only_map), occupancy-weighted to avoid bias from bins some cells
    oversampled relative to others. Not normalized to each cell's own peak (see
    pool_field_only_map), so values are in raw Hz."""
    fig = plt.figure(figsize=(15, 5))
    for i, key in enumerate(_ARENA_ORDER):
        handler = arena_handlers[key]
    ...
    fig.suptitle('Field-only mean rate maps (occupancy-weighted, place-field bins only, unnormalized)')
    fig.tight_layout()
    fig.savefig(save_path, dpi=200)
    plt.close(fig)
    print(f'[SAVED] {save_path}')

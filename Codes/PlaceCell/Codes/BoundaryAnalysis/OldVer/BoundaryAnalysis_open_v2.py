...
root_folder  = r'C:/Runita/NMR/analysis/AllSort_Results/PlaceCell/Data/Fa8477/Open'
output_excel = r'C:/Runita/NMR/analysis/AllSort_Results/PlaceCell/Data/Fa8477/Open/Open_BoundaryAnalysis_v2.xlsx'
...
def _load_and_bin_tracking(csv_path: str, arena_width_cm: float, target_bin_cm: float,
    ...
    Returns (t, beh_bx, beh_by, n_bins_x, n_bins_y) or None if no samples remain.
    """
    ...
    return t, beh_bx, beh_by, n_bins_x, n_bins_y
...
def _zone_occupancy_pct(occ_map: np.ndarray, valid_mask: np.ndarray,
                        n_bins_x: int, n_bins_y: int,
                        target_bin_cm: float, arena_diameter_cm: float,
    ...
    edge_mask = (dist_from_wall_cm <= edge_threshold_cm) & valid_mask
...
def compute_session_zone_occupancy(csv_path: str, arena_width_cm: float, target_bin_cm: float):
    ...
    if track is None:
        return None
    t, beh_bx, beh_by, n_bins_x, n_bins_y = track
    ...
    edge_pct, centre_pct = _zone_occupancy_pct(occ_map, valid_mask, n_bins_x, n_bins_y,
                                               target_bin_cm, arena_width_cm)
    return edge_pct, centre_pct, occ_map, valid_mask, n_bins_x, n_bins_y
...
def _plot_occupancy_map(occ_map: np.ndarray, valid_mask: np.ndarray,
    ...
    arena_radius_cm  = arena_diameter_cm / 2.0
    inner_radius_cm  = max(0.0, arena_radius_cm - edge_threshold_cm)
    ...
    for dirpath, _, filenames in os.walk(root_folder):
        ...
        if len(tracking_files) == 1 and len(ntt_files) > 0:
            ...
            if occ_result is not None:
                edge_pct, centre_pct, occ_map, valid_mask, _n_bx, _n_by = occ_result
                print(f'  {session_name}: edge zone = {edge_pct:.1f}%   centre zone = {centre_pct:.1f}%')
                try:
                    _plot_occupancy_map(occ_map, valid_mask, target_bin_cm, arena_width_cm,

...
OUTPUT_DIR = r'X:\NMR_group_data\Runita\Analysis\Thesis\Data_v2_Accepted\MeanRM_Quad_woRot'
...
def plot_quadrant_kde(arena_handlers: dict, results: dict, save_path: str):
    ...
    for mi, map_type in enumerate(_MAP_ORDER):
        ...
        # colour scales are built from the genuinely sampled bins only (quad_valid) -- the
        # zero-filled unsampled bins would otherwise pin vmin at 0 and compress the range
        main_vals = np.concatenate([results[k][map_type]['real_med'][results[k][map_type]['quad_valid']]
        ...
        diffs = {k: results[k][map_type]['real_med'] - results[k][map_type]['null_kde']
                 for k in _ARENA_ORDER}
        diff_all = np.concatenate([diffs[k][results[k][map_type]['quad_valid']] for k in _ARENA_ORDER])
        ...
        for ci, arena_key in enumerate(_ARENA_ORDER):
            ...
            # isfinite() is True everywhere and those bins would render at vmin (see
            # _quadrant_data_mask) instead of being left blank
            main_valid = res['quad_valid'] & np.isfinite(res['real_med'])
            ...
            diff_valid = res['quad_valid'] & np.isfinite(res['real_med']) & np.isfinite(res['null_kde'])

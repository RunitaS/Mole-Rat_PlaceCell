...
The track is a thin circular alley (~4 cm wide), not an open field. A
Cartesian (x, y) grid of square bins does not align with the track's local
radial/tangential geometry: each square bin only partially, and unevenly,
overlaps the curved path, which starves some bins of occupancy even under
uniform running and fragments true fields wherever an under-occupied bin
happens to span the full width of the track. To avoid this, the rate map
is built directly in **polar coordinates (r, theta)** about the track's
fitted centre: one bin axis follows the track's radial (width) direction,
the other follows its angular (running direction) axis, so bins line up
with the track's actual shape instead of cutting across it. The angular
axis wraps around (bin 0 borders the last bin) since the track is a closed
loop; the radial axis does not (it has real inner/outer edges).

Algorithm (per .ntt file) – "threshold method", adapted from a MATLAB
`placefield`/`getLegals` reference implementation:
    1. Keep only occupied (r, theta) bins whose smoothed firing rate is both
       >= METHOD2_RATE_THRESHOLD_FRAC of the cell's peak rate AND above
       the cell's mean firing rate.
    2. 8-connected-component label the surviving bins, wrapping around the
       angular axis (theta=0 is adjacent to theta=2*pi).
...
def _gaussian_smooth_polar(fr_map: np.ndarray, valid_mask: np.ndarray, bin_cm: float) -> np.ndarray:
    """Gaussian smoothing on an (r, theta) rate map. The angular axis (axis 1)
    is circular -- the track is a closed loop -- so it is padded by wrapping
    rather than zero-filling, which would otherwise create a spurious rate
    dip at the arbitrary theta=0/2*pi seam. The radial axis (axis 0) has real
    inner/outer edges, so it is zero-padded as before."""
    ...
    def _pad(arr, xp):
        arr = xp.pad(arr, ((kr, kr), (0, 0)), mode='constant', constant_values=0.0)
        arr = xp.pad(arr, ((0, 0), (kr, kr)), mode='wrap')
        return arr

    n_r, n_theta = fr_map.shape
    ...
    smoothed_fr      = smoothed_fr_full[kr:kr + n_r, kr:kr + n_theta]
    smoothed_weights = smoothed_weights_full[kr:kr + n_r, kr:kr + n_theta]
...
# ── Ring-track geometry (polar binning) ─────────────────────────────────────────

RADIAL_RANGE_CLIP_PCTILE = 0.5   # trim this many percentiles off each end of the
                                  # radial-distance distribution before setting the
                                  # radial bin range, so a handful of tracking-jitter
                                  # outliers can't stretch/blur the bin edges
...
def build_ratemap(csv_path: str, ntt_path: str,
    ...
    valid_mask, spike_frame indices, t, beh_br, beh_bt, n_bins_r/theta. Bins
    are built in polar (r, theta) coordinates about the fitted ring centre
    (see module docstring) rather than Cartesian (x, y), so bins follow the
    track's actual radial/angular geometry instead of cutting across it."""
    ...
    r_span = max(r_max - r_min, target_bin_cm)
    n_bins_r = max(1, int(np.ceil(r_span / target_bin_cm)))
    ...
    beh_br = np.clip(((r_cm - r_min) / target_bin_cm).astype(int), 0, n_bins_r - 1)
    ...
    occ_map   = np.zeros((n_bins_r, n_bins_theta), dtype=np.float64)
    spike_map = np.zeros((n_bins_r, n_bins_theta), dtype=np.float64)

    np.add.at(occ_map,   (beh_br, beh_bt), dt_frames)
    np.add.at(spike_map, (sp_br,  sp_bt),  1.0)

    valid_mask = occ_map >= min_occ_s

    fr_raw = np.zeros_like(occ_map)
    fr_raw[valid_mask] = spike_map[valid_mask] / occ_map[valid_mask]
    fr_smooth = _gaussian_smooth_polar(fr_raw, valid_mask, target_bin_cm)

    # Physical area (cm^2) of each (r, theta) bin -- arc length grows with r,
    # so a polar bin's area is r_bin_centre * bin_width_rad * target_bin_cm,
    # not a flat target_bin_cm^2 as it would be for a square Cartesian bin.
    r_bin_centres = r_min + (np.arange(n_bins_r) + 0.5) * target_bin_cm
    bin_area_map  = np.broadcast_to(
        (r_bin_centres * bin_width_rad * target_bin_cm)[:, None],
        (n_bins_r, n_bins_theta)
    )

    ctx = dict(spike_ts=spike_ts[valid_spike], spike_frame=spike_frame, t=t,
               beh_br=beh_br, beh_bt=beh_bt,
               occ_map=occ_map, valid_mask=valid_mask,
               fr_raw=fr_raw, fr_smooth=fr_smooth,
               n_bins_r=n_bins_r, n_bins_theta=n_bins_theta,
               r_min=r_min, r_max=r_max, bin_width_rad=bin_width_rad,
               bin_area_map=bin_area_map, cx=cx, cy=cy)
...
def _connected_components_8_polar(qualifies: np.ndarray, n_bins_r: int, n_bins_theta: int) -> list:
    """8-connected component labelling of the bins where `qualifies` is True,
    on an (r, theta) grid. The angular axis (theta) wraps around -- bin 0 and
    bin n_bins_theta-1 are neighbours, since the track is a closed loop -- so
    a field is never artificially split just because it straddles the
    arbitrary theta=0/2*pi seam. The radial axis does not wrap (it has real
    inner/outer edges)."""
    visited = ~qualifies
    components = []
    for i in range(n_bins_r):
        for j in range(n_bins_theta):
            if visited[i, j]:
                continue
            region = []
            stack = [(i, j)]
                ...
                for ddx in (-1, 0, 1):
                    for ddy in (-1, 0, 1):
                        if ddx == 0 and ddy == 0:
                            continue
                        nr = br + ddx
                        if not (0 <= nr < n_bins_r):
                            continue
                        nt = (bt + ddy) % n_bins_theta
...
def detect_place_fields_threshold(base_metrics: dict, ctx: dict, target_bin_cm: float) -> list[dict]:
    ...
    components of qualifying (r, theta) bins spanning >= MIN_FIELD_SIZE_BINS
    contiguous bins (no discontinuity, wrapping around the angular seam) are
    reported as fields (peak bin + rate-weighted centre of mass, mirroring
    the reference's `fieldPos`/`centreFieldSize`)."""
    valid_mask   = ctx['valid_mask']
    fr_smooth    = ctx['fr_smooth']
    n_bins_r     = ctx['n_bins_r']
    n_bins_theta = ctx['n_bins_theta']
    r_min        = ctx['r_min']
    bin_width_rad = ctx['bin_width_rad']
    bin_area_map = ctx['bin_area_map']
    cx, cy       = ctx['cx'], ctx['cy']
    ...
    total_occupied_area_cm2 = float(bin_area_map[valid_mask].sum())

    # Legacy "centre field" heuristic (nearest field to the middle of the
    # bin grid) kept for output-schema continuity. On a ring track this no
    # longer denotes proximity to the arena's geometric centre (the true
    # centre lies inside the ring, off the (r, theta) grid entirely) -- it
    # just flags the field nearest the middle of the radial range at the
    # arbitrary theta=0 reference bearing.
    centre_ref_r = n_bins_r / 2.0
    best_centre_dist  = np.inf
    centre_field_idx  = None
    fields = []

    for region in _connected_components_8_polar(qualifies, n_bins_r, n_bins_theta):
        if len(region) < min_size_bins:
            continue

        brs   = np.array([b[0] for b in region])
        bts   = np.array([b[1] for b in region])
        rates = fr_smooth[brs, bts]

        peak_local_idx = int(np.argmax(rates))
        peak_br, peak_bt = int(brs[peak_local_idx]), int(bts[peak_local_idx])
        peak_val = float(rates[peak_local_idx])
        peak_r_cm      = r_min + (peak_br + 0.5) * target_bin_cm
        peak_theta_rad = (peak_bt + 0.5) * bin_width_rad

        r_centers     = r_min + (brs + 0.5) * target_bin_cm
        theta_centers = (bts + 0.5) * bin_width_rad

        total_rate = float(rates.sum())
        com_r_cm    = float(np.sum(rates * r_centers) / total_rate)
        ...
        area_cm2     = float(bin_area_map[brs, bts].sum())
        pct_area     = 100.0 * area_cm2 / total_occupied_area_cm2 if total_occupied_area_cm2 > 0 else 0.0
        bin_coords_str = ';'.join(f'{br}-{bt}' for br, bt in sorted(region))

        com_cm_x = cx + com_r_cm * np.cos(com_theta_rad)
        com_cm_y = cy + com_r_cm * np.sin(com_theta_rad)
        peak_cm_x = cx + peak_r_cm * np.cos(peak_theta_rad)
        peak_cm_y = cy + peak_r_cm * np.sin(peak_theta_rad)

        # Circular distance in bin units (theta wraps, r doesn't) for the
        # legacy centre-field heuristic.
        ...
        dist_to_centre = float(np.hypot((com_r_cm - r_min) / target_bin_cm - centre_ref_r, dtheta_bins))
        ...
        fields.append({
            ...
            'peak_r_cm':             round(peak_r_cm, 2),
            'peak_theta_deg':        round(np.degrees(peak_theta_rad) % 360.0, 2),
            'peak_cm_x':             round(float(peak_cm_x), 2),
            'peak_cm_y':             round(float(peak_cm_y), 2),
            'peak_fr_hz':            round(peak_val, 4),
            'com_r_cm':              round(com_r_cm, 2),
            'com_theta_deg':         round(np.degrees(com_theta_rad) % 360.0, 2),
            'com_cm_x':              round(float(com_cm_x), 2),
            'com_cm_y':              round(float(com_cm_y), 2),
            'n_bins':                n_bins_field,
            'area_cm2':              round(area_cm2, 2),
            'pct_of_occupied_area':  round(pct_area, 2),
            'bbox_r_min':            int(brs.min()),
            'bbox_r_max':            int(brs.max()),
...
def _field_mask_from_bin_coords(bin_coords: str, n_bins_r: int, n_bins_theta: int) -> np.ndarray:
    mask = np.zeros((n_bins_r, n_bins_theta), dtype=bool)
    if not bin_coords:
        return mask
    for pair in bin_coords.split(';'):
        br_str, bt_str = pair.split('-')
        mask[int(br_str), int(bt_str)] = True
    return mask


def _plot_ratemap_with_field_boundaries(ax, fr_smooth: np.ndarray, valid_mask: np.ndarray,
                                         fields: list[dict], n_bins_r: int, n_bins_theta: int,
                                         r_min: float, target_bin_cm: float, bin_width_rad: float,
                                         title: str):
    """Plots the (r, theta) rate map on a polar axis, so the ring track
    renders as an actual ring: theta=0 and theta=2*pi coincide in physical
    space, so a field that straddles that seam still appears as one
    unbroken patch -- no wraparound artifact, unlike a flat (x, y) grid."""
    display_map = np.ma.masked_where(~valid_mask, fr_smooth)

    theta_edges = np.arange(n_bins_theta + 1) * bin_width_rad
    r_edges     = r_min + np.arange(n_bins_r + 1) * target_bin_cm
    Theta_e, R_e = np.meshgrid(theta_edges, r_edges)
    im = ax.pcolormesh(Theta_e, R_e, display_map, cmap='jet', shading='flat')

    # Field-boundary contours: pad one extra column on each side of the
    # angular axis by wrapping, so contour lines close smoothly across the
    # seam instead of stopping dead at the first/last theta column.
    theta_centers = (np.arange(n_bins_theta) + 0.5) * bin_width_rad
    r_centers     = r_min + (np.arange(n_bins_r) + 0.5) * target_bin_cm
    theta_c_pad = np.concatenate([theta_centers[-1:] - 2.0 * np.pi, theta_centers, theta_centers[:1] + 2.0 * np.pi])
    Theta_c_pad, R_c_pad = np.meshgrid(theta_c_pad, r_centers)

    for field in fields:
        field_mask = _field_mask_from_bin_coords(field.get('bin_coords', ''), n_bins_r, n_bins_theta)
        if not field_mask.any():
            continue
        field_mask_pad = np.concatenate([field_mask[:, -1:], field_mask, field_mask[:, :1]], axis=1)
        ax.contour(Theta_c_pad, R_c_pad, field_mask_pad.astype(float), levels=[0.5], colors='black', linewidths=1.5)
...
def _save_field_ratemap_plot(ctx: dict, fields_threshold: list[dict], ntt_path: str, target_bin_cm: float):
    """Saves one PNG per .ntt file: polar rate map with detected-field
    boundaries (black outlines) from the threshold method."""
    fr_smooth     = ctx['fr_smooth']
    valid_mask    = ctx['valid_mask']
    n_bins_r      = ctx['n_bins_r']
    n_bins_theta  = ctx['n_bins_theta']
    ...
    im = _plot_ratemap_with_field_boundaries(ax, fr_smooth, valid_mask, fields_threshold,
                                              n_bins_r, n_bins_theta, r_min, target_bin_cm, bin_width_rad,
                                              f'Threshold method ({len(fields_threshold)} field(s))')
    fig.colorbar(im, ax=ax, label='Hz', shrink=0.75)
    fig.tight_layout()
...
def _run_job(args):
    ...
    try:
        _save_field_ratemap_plot(ctx, fields_threshold, ntt_path, target_bin_cm)
    except Exception as e:
        with _print_lock:
...
if __name__ == "__main__":
    ...
    field_columns_threshold = [
        'field_number', 'peak_bin_r', 'peak_bin_theta', 'peak_r_cm', 'peak_theta_deg',
        'peak_cm_x', 'peak_cm_y', 'peak_fr_hz',
        'com_r_cm', 'com_theta_deg', 'com_cm_x', 'com_cm_y',
        'n_bins', 'area_cm2', 'pct_of_occupied_area',
        'bbox_r_min', 'bbox_r_max', 'theta_span_deg',
        'bbox_theta_min_deg', 'bbox_theta_max_deg', 'wraps_theta_seam',

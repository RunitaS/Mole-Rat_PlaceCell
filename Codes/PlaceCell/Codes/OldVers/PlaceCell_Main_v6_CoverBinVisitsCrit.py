...
# ── Configuration ─────────────────────────────────────────────────────────────
root_folder  = r'X:\NMR_group_data\Runita\Analysis\Thesis\Data_v2_Accepted'
output_excel = r'X:\NMR_group_data\Runita\Analysis\Thesis\Data_v2_Accepted\All_TT_PlaceChar_1sec.xlsx'
...
Output_PlaceTrue = r'X:\NMR_group_data\Runita\Analysis\Thesis\Data_v2_Accepted\All_TT_PlaceTrue'

# root_folder  = r'C:/Runita/NMR/analysis/AllSort_Results/PlaceCell/Data/PlaceCell_True_v2/Fa23BD/Day5'
# output_excel = r'C:/Runita/NMR/analysis/AllSort_Results/PlaceCell/Data/PlaceCell_True_v2/TT_PlaceChar.xlsx'

# # Destination for .ntt + tracking files of confirmed place cells (folder pattern
# # replicated from the animal-ID folder onwards, e.g. Fa1059/Open/<session>/...)
# Output_PlaceTrue = r'C:/Runita/NMR/analysis/AllSort_Results/PlaceCell/Data/PlaceCell_True_v2/PlaceTrue'
...
def _compute_speed_modulation(
    ...
    if fig_combined is not None:
        ...
        save_dir   = os.path.join(os.path.dirname(ntt_path), 'speed modulation_VisitCrit')
...
def _run_bootstrap(spike_frame_indices: np.ndarray, t: np.ndarray,
    ...
    ntt_name   = os.path.splitext(os.path.basename(ntt_path))[0]
    save_dir   = os.path.join(os.path.dirname(ntt_path), 'shuffling_VisitCrit')
    os.makedirs(save_dir, exist_ok=True)
    lbl_suffix = f'_{label}' if label else ''
...
def _plot_and_save_ratemap(fr_map: np.ndarray, valid_mask: np.ndarray,
    ...
    save_dir  = os.path.join(os.path.dirname(ntt_path), 'ratemaps_VisitCrit')
    os.makedirs(save_dir, exist_ok=True)
...
def _plot_and_save_split_half_ratemap(ctx: dict, stability: dict,
    ...
    First/second-half occupancy and spike maps are rebuilt here on the SAME
    full-session grid used by `_compute_split_half_stability` (not the
    independently-binned first/second-half rows computed elsewhere), so this
    plot is bin-aligned with, and visually explains, the reported stability
    score.
    """
    t           = ctx['t']
    beh_bx      = ctx['beh_bx']
    ...
    # Gaussian-smoothed for display, matching the full-session ratemap
    # (ctx['fr_smooth']) so all three panels are visually comparable --
    # the stability score itself is computed on the raw (unsmoothed) maps
    # elsewhere, in _compute_split_half_stability.
    fr_first_smooth  = _gaussian_smooth(fr_first_raw,  valid_first,  target_bin_cm)
    fr_second_smooth = _gaussian_smooth(fr_second_raw, valid_second, target_bin_cm)
    ...
    ax_second = fig.add_subplot(gs[1, 1])
    ...
    fig.colorbar(im, ax=ax_full, label='firing rate (Hz)', fraction=0.03)
    ...
    if full_ctx and full_ctx.get('valid_mask') is not None and full_ctx['valid_mask'].any():
        try:
            ...
            _plot_and_save_ratemap(
                ...
                ntt_path, full_row, full_ctx['x_cm'], full_ctx['y_cm'], full_ctx['spike_frame'],

...
Algorithm (per .ntt file):
    ...
    Part 1 – detect the current highest place field
        1. Find the bin with the peak (smoothed) firing rate (must be above
           the cell's current mean firing rate).
        2. Region-grow (8-connected) to all neighbouring occupied bins whose
           firing rate is >= 20% of that peak bin's rate AND above the cell's
           current mean firing rate. Bins at or below the mean are never
           included in a field.
    ...
    Part 2 – iterate to find every field
        5. Set every bin belonging to already-detected fields to NaN, removing
           them from the occupied/valid-bin set entirely.
        6. Recompute SIR / sparsity / peak / mean firing rate and rerun the
           location-shuffling bootstrap on this remaining rate map (with
           previously detected fields excluded).
        7. If the cell is still spatially selective (same place-cell criteria
           ...
           remaining map to isolate the next field. Otherwise stop.

    "Threshold method" (per .ntt file) – a second, independent detector run
    alongside the one above, adapted from a MATLAB `placefield`/`getLegals`
    reference implementation:
        ...
        Unlike the tedious method, this pass is a single non-iterative sweep
        (no suppression / re-bootstrapping).
...
# ── Bootstrap (location-shuffle); pass a restricted valid_mask to exclude bins ──
...
def _sir_from_spikes_locshuf(spike_frame_indices: np.ndarray, rnd: int,
    ...
    assignment fixed). `valid_mask` should already exclude any bins that are
    to be left out of the analysis (e.g. previously isolated place fields),
    so the shuffle is computed on the same remaining rate map as the real
    data."""
    n_frames   = len(beh_bx)
...
def _flood_fill(seed, fr_map, valid_mask, field_mask, threshold, mean_fr, n_bins_x, n_bins_y):
    visited = set()
    stack = [seed]
    visited.add(seed)
    while stack:
        ...
        for ddx in (-1, 0, 1):
            for ddy in (-1, 0, 1):
                ...
                if nb in visited or not valid_mask[nx, ny] or field_mask[nx, ny]:
                    continue
                if fr_map[nx, ny] >= threshold and fr_map[nx, ny] > mean_fr:
                    visited.add(nb)
                    stack.append(nb)
    return visited


def _find_one_field(fr_map, valid_mask, field_mask, min_size_bins, mean_fr, n_bins_x, n_bins_y):
    """Part 1, steps 1-3: peak bin -> region grow -> size check -> next-highest
    bin on failure. Returns the winning set of (bx, by) bins, or None. Bins at
    or below the cell's mean firing rate are never included in a field."""
    ...
    for k in order:
        ...
        if peak_val <= 0 or peak_val <= mean_fr:
            break
        threshold = FIELD_RATE_FRAC * peak_val
        region = _flood_fill(seed, fr_map, valid_mask, field_mask, threshold, mean_fr, n_bins_x, n_bins_y)
...
def detect_place_fields(base_metrics: dict, ctx: dict, ntt_path: str,
    ...
    remaining_valid  = valid_mask.copy()
    current_mean_fr  = float(base_metrics.get('mean_fr', 0.0))
    fields           = []
    field_num        = 1

    while True:
        region = _find_one_field(current_fr, remaining_valid, field_mask, min_size_bins,
                                  current_mean_fr, n_bins_x, n_bins_y)
        if region is None:
            with _print_lock:
        ...
        # ── remove this field's bins (set to NaN) and re-test spatial selectivity
        #    on the remaining rate map ───────────────────────────────────────────
        for b in region:
            field_mask[b[0], b[1]] = True
        suppressed_fr = current_fr.copy()
        suppressed_fr[field_mask] = np.nan
        remaining_valid = valid_mask & ~field_mask

        after = _metrics_from_ratemap(suppressed_fr, occ_map, remaining_valid)
        boot  = _run_bootstrap(spike_frame, t, beh_bx, beh_by, occ_map, remaining_valid,
...
def _save_field_ratemap_plot(ctx: dict, fields: list[dict], fields_threshold: list[dict], ntt_path: str):
    """Saves one PNG per .ntt file: rate map with detected-field boundaries
    (black outlines) for the tedious method (top) and the threshold method
    (bottom), stacked in the same image."""
    fr_smooth  = ctx['fr_smooth']
    valid_mask = ctx['valid_mask']
    n_bins_x   = ctx['n_bins_x']
    n_bins_y   = ctx['n_bins_y']

    fig = Figure(figsize=(6, 11))
    canvas = FigureCanvasAgg(fig)
    ax1 = fig.add_subplot(211)
    ax2 = fig.add_subplot(212)

    im1 = _plot_ratemap_with_field_boundaries(ax1, fr_smooth, valid_mask, fields,
                                               n_bins_x, n_bins_y,
                                               f'Tedious method ({len(fields)} field(s))')
    im2 = _plot_ratemap_with_field_boundaries(ax2, fr_smooth, valid_mask, fields_threshold,
                                               n_bins_x, n_bins_y,
                                               f'Threshold method ({len(fields_threshold)} field(s))')
    fig.colorbar(im1, ax=ax1, label='Hz')
    fig.colorbar(im2, ax=ax2, label='Hz')
    fig.tight_layout()
...
def _run_job(args):
    ...
    try:
        ...
        with _print_lock:
            print(f'  ERROR detecting fields (tedious method) in {ntt_file}: {e}')
        return session_name, ntt_file, [], [], {'error': str(e)}
    ...
    try:
        _save_field_ratemap_plot(ctx, fields, fields_threshold, ntt_path)
    except Exception as e:
        with _print_lock:
    ...
    return session_name, ntt_file, fields, fields_threshold, summary
...
if __name__ == "__main__":
    ...
    for session_name, ntt_file, fields, fields_threshold, summary in sorted_results:
        sheet_name           = _safe_sheet_name(os.path.splitext(ntt_file)[0], used_sheet_names)
        sheet_name_threshold = _safe_sheet_name(os.path.splitext(ntt_file)[0] + '_Thr', used_sheet_names)
        ...
        summary_rows.append({
            ...
            'sheet_name_threshold': sheet_name_threshold,
            'n_fields_detected_threshold': len(fields_threshold),
            'field_areas_cm2_threshold':  '; '.join(f'{a:.2f}' for a in areas_thr),
            'field_pct_areas_threshold':  '; '.join(f'{p:.2f}' for p in pcts_thr),
            'total_field_area_cm2_threshold': round(sum(areas_thr), 2) if areas_thr else 0.0,
            'total_pct_area_occupied_threshold': round(sum(pcts_thr), 2) if pcts_thr else 0.0,
    ...
    summary_columns = [
        ...
        'sheet_name_threshold', 'n_fields_detected_threshold',
        'field_areas_cm2_threshold', 'field_pct_areas_threshold',
        'total_field_area_cm2_threshold', 'total_pct_area_occupied_threshold',
    ...
    with pd.ExcelWriter(output_excel, engine='openpyxl') as writer:
        ...
        for (session_name, ntt_file, fields, fields_threshold, summary), row in zip(sorted_results, summary_rows):
            ...
            sheet_name_threshold = row['sheet_name_threshold']
            if fields_threshold:
                df_thr = pd.DataFrame(fields_threshold, columns=field_columns_threshold)
            else:
                df_thr = pd.DataFrame(columns=field_columns_threshold)
            df_thr.to_excel(writer, sheet_name=sheet_name_threshold, index=False)

    print(f'\nDone. Results saved to {output_excel}')
    print(f'Total units processed : {len(results)}')
    print(f'Total fields detected (tedious method)   : {sum(r["n_fields_detected"] for r in summary_rows)}')
    print(f'Total fields detected (threshold method) : {sum(r["n_fields_detected_threshold"] for r in summary_rows)}')

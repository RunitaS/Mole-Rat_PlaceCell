...
root_folder  = r'C:/Runita/NMR/analysis/AllSort_Results/PlaceCell/Data/PlaceCell_True_v2/Fa1059/AdptBin_Roddy'
output_excel = r'C:/Runita/NMR/analysis/AllSort_Results/PlaceCell/Data/PlaceCell_True_v2/Fa1059/AdptBin_Roddy/Test_AdptBin_Roddy.xlsx'
...
Output_PlaceTrue = r'C:/Runita/NMR/analysis/AllSort_Results/PlaceCell/Data/PlaceCell_True_v2/Fa1059/AdptBin_Roddy'
fps            = 30           # tracking frame rate (Hz)
target_bin_cm  = 2.0          # bin size in cm
...
ADAPTIVE_ALPHA_BASE = 0.0001  # "a" in the Skaggs & McNaughton (1998) adaptive-binning
                               # criterion r >= a / (n * sqrt(s)); applied directly to raw
                               # occupancy SAMPLE COUNTS (n, via occ_count_map below), not
                               # occupancy time -- matches Grieves rate_mapper.m 'adaptive'
ADAPTIVE_MINDIST_CM  = 4.0    # checkpoint radius (cm): if the occupancy TIME enclosed within
                               # this fixed radius doesn't reach min_occ_s, the bin is invalid
                               # outright, regardless of how far the search would otherwise
                               # expand (Grieves default ratio mindist/binsize = 40mm/20mm = 2 bins)
ADAPTIVE_MAXDIST_CM  = 64.0   # max search radius (cm) before an adaptively binned bin gives up
                               # expanding and uses whatever it accumulated at the cap
                               # (Grieves default ratio maxdist/binsize = 640mm/20mm = 32 bins)
...
def _adaptive_binned_ratemap(occ_map: np.ndarray, spike_map: np.ndarray,
    ...
    rsq * n^2 * s >= a^-2, which is what's evaluated below (rsq = r^2, tested
    at every integer bin^2 step so the true minimum satisfying radius is
    always found, rather than Grieves' coarser log-spaced `steps` grid).
    Once the radius is chosen, the bin's rate is spikes / occupancy-TIME
    (occ_map, in seconds) enclosed within it.

    Unlike a naive port, a bin with zero spikes in its neighbourhood is never
    substituted with "1 spike" to dodge a zero threshold -- the criterion
    simply never triggers while s=0 (rsq*n^2*0 is never >= a^-2), so the
    neighbourhood keeps growing until a spike is found or maxdist is hit,
    exactly as in rate_mapper.m (there, s=0 makes the RHS infinite).
    ...
    Two validity gates, both taken directly from rate_mapper.m:
      - mindist checkpoint: if the occupancy TIME enclosed within a small
        ...
        outright. Without this, a bin could always eventually accumulate
        min_occ_s by expanding far enough -- which would validate estimates
        for locations with no real local coverage (e.g. outside the area the
        animal actually explored) as long as *some* data existed anywhere
        within maxdist.
      - maxdist cap: the search stops at `maxdist_cm`; if the criterion was
        ...
        to that cap (same fallback as rate_mapper.m's `if isempty(rindx)`).

    Every bin within reach of some visited bin is evaluated, not just bins
    with nonzero occupancy of their own -- an unvisited bin can still borrow
    a valid estimate from nearby visited bins, matching rate_mapper.m, which
    loops over every bin centre in the map rather than only directly-visited
    ones. Bins with no visited bin within maxdist are skipped outright (via a
    distance transform on the occupancy mask) since they are guaranteed to
    fail the mindist checkpoint below anyway (mindist <= maxdist) -- this is
    the vectorized equivalent of rate_mapper.m's own bounding-box early exit
    ("if no position data within maxdist, leave NaN").
    """
    ...
    # smallest tested radius is a full bin width (r=1), not r=0, so even the
    # most densely sampled bin still pools at least its immediate neighbours.
...
def _sir_and_coherence_from_spikes_locshuf(spike_frame_indices: np.ndarray, rnd: int,
    ...
    iteration only needs one shuffled spike train. This deliberately reuses
    the cheap fixed-bin rate map (rather than the adaptively-binned map the
    real-data SIR/coherence in compute_metrics now use) as an approximation
    of the null distribution -- re-running adaptive binning on every one of
    the N_BOOTSTRAP shuffles per cell would be far too slow for batch use.
    """
    n_frames   = len(beh_bx)
...
def _plot_and_save_ratemap(fr_map: np.ndarray, valid_mask: np.ndarray,
    ...
    adaptive-binned firing-rate map for one cell, and save both next to its
    .ntt file, in a 'ratemaps' folder (mirrors the 'speed modulation' folder
    convention above).
    """
...
def _compute_split_half_stability(ctx: dict,
    ...
    """Split-half spatial stability: Pearson correlation of the adaptively
    binned firing-rate map between the first and second half of the session.
    ...
    Bin validity handling: each half's rate map is built with
    _adaptive_binned_ratemap (same estimator as the full-session ratemap),
    so a bin is valid for a half once its adaptively-expanded neighbourhood
    reaches min_occ_s of occupancy in that half, rather than requiring the
    bin's own un-expanded dwell time to clear that bar. A bin is only
    correlated if it is valid in BOTH halves -- the standard split-half
    approach in the place-cell literature. If fewer than `min_valid_bins`
    bins survive this, the score is left as NaN rather than reported from
    too few points.
    """
    ...
    occ_count_first  = np.zeros((n_bins_x, n_bins_y), dtype=np.float64)
    occ_count_second = np.zeros((n_bins_x, n_bins_y), dtype=np.float64)
    np.add.at(occ_count_first,  (beh_bx[:mid], beh_by[:mid]), 1.0)
    np.add.at(occ_count_second, (beh_bx[mid:], beh_by[mid:]), 1.0)
    ...
    fr_first_map,  valid_first  = _adaptive_binned_ratemap(
        occ_first,  spike_first,  occ_count_first,  min_occ_s, fps, target_bin_cm)
    fr_second_map, valid_second = _adaptive_binned_ratemap(
        occ_second, spike_second, occ_count_second, min_occ_s, fps, target_bin_cm)
    ...
    fr_first  = fr_first_map[common_valid]
    fr_second = fr_second_map[common_valid]
...
def compute_metrics(csv_path: str, ntt_path: str,
    ...
    # Fixed-bin occupancy mask, kept only for the SIR/coherence location-
    # shuffling bootstrap (_run_bootstrap), which reuses this raw grid on
    # every one of its N_BOOTSTRAP shuffles and stays on the cheap fixed-bin
    # estimator rather than re-running adaptive binning that many times.
    valid_mask = occ_map >= min_occ_s

    # ── 6-7. Adaptive-binned firing rate map ───────────────────────────────────
    # Replaces the fixed-bin histogram + triangular smoothing: each bin's
    # neighbourhood radius grows until it encloses enough occupancy and
    # spikes to satisfy the Skaggs & McNaughton (1998) adaptive-binning
    # criterion, ported from Roddy Grieves' rate_mapper.m 'adaptive' method
    # (github.com/RoddyMGrieves/rate_mapper) -- see _adaptive_binned_ratemap.
    fr_adaptive, valid_mask_adaptive = _adaptive_binned_ratemap(
        occ_map, spike_map, occ_count_map, min_occ_s, fps, target_bin_cm)

    # Raw fixed-bin (2x2 cm) firing-rate map, used below for peak_fr/sir so
    # those two metrics reflect the un-smoothed per-bin spike/occupancy
    # counts rather than the adaptively-interpolated estimate.
    fr_raw = np.zeros_like(spike_map)
    np.divide(spike_map, occ_map, out=fr_raw, where=valid_mask)
    ...
    if not valid_mask_adaptive.any():
        return ({'n_spikes': n_spikes, 'n_discarded': n_discarded,
                 'peak_fr': 0.0, 'mean_fr': 0.0, 'sir': 0.0,
                 'sparsity': 0.0, 'coherence': float('nan')}, ctx)

    total_occ_s = occ_map[valid_mask_adaptive].sum()
    pi_flat     = occ_map[valid_mask_adaptive] / total_occ_s
    ri_flat     = fr_adaptive[valid_mask_adaptive]
    r_mean      = float(np.sum(pi_flat * ri_flat))

    mean_fr = r_mean

    # Peak firing rate and SIR from the raw fixed-bin (2x2 cm) map rather
    # than the adaptively-binned map used for mean_fr/sparsity/coherence.
    total_occ_s_raw = occ_map[valid_mask].sum()
    pi_flat_raw     = occ_map[valid_mask] / total_occ_s_raw
    ri_flat_raw     = fr_raw[valid_mask]
    r_mean_raw      = float(np.sum(pi_flat_raw * ri_flat_raw))

    peak_fr = float(ri_flat_raw.max())

    sir = 0.0
    if r_mean_raw > 0:
        nonzero = ri_flat_raw > 0
        ratio   = ri_flat_raw[nonzero] / r_mean_raw
        sir     = float(np.sum(pi_flat_raw[nonzero] * ratio * np.log2(ratio)))
    ...
    # Spatial coherence: adaptively-binned map vs 8-neighbour mean (Fisher Z)
    coherence = _compute_coherence(fr_adaptive, valid_mask_adaptive, n_bins_x, n_bins_y)
    ...
    # Rate-map plot -- full session only (not run on first/second half splits).
    if half is None:
        try:
            _plot_and_save_ratemap(fr_adaptive, valid_mask_adaptive, n_bins_x, n_bins_y,
                                    target_bin_cm, ntt_path, metrics,
                                    x_cm, y_cm, spike_frame)
        except Exception as e:
            print(f'  RATEMAP PLOT ERROR in {os.path.basename(ntt_path)}: {e}')

...
def generalized_phase_vector(x, fs, lp, nwin=3):
    ...
    wt_post : instantaneous frequency of the CORRECTED phase (recomputed
         from xgp after the phase-slip stitching below, same rate units as
         fs). This is the one that should stay close to the passband; a
         large discrepancy from wt_pre in a given epoch is exactly where GP
    ...
    if np.all(np.isnan(ph)):
        return np.full(npts, np.nan, dtype=np.complex128), wt_raw, np.full(npts, np.nan)
    ...
    # "stitch over" those epochs: unwrap only the *trustworthy* (unflagged)
    # samples, treating them as a contiguous sequence -- so a brief flagged
    # epoch never introduces a false +/-2*pi jump between the samples on
    # either side of it -- then reconstruct the flagged samples by
    # shape-preserving (pchip) interpolation from that trustworthy unwrapped
    # trend, and rewrap.
    #
    # (generalized_phase_vector.m instead calls MATLAB's unwrap() on the
    # full array with the flagged epoch set to NaN. Because unwrap()'s phase
    # correction is a running cumulative sum, one NaN run poisons every
    # sample after it (real + NaN = NaN), so literally translated, the
    # *entire remainder* of the trace would be discarded and pchip
    # *extrapolated* from the trend before the very first flagged epoch.
    # That is invisible for the short (<1 s), essentially single-epoch
    # fixation trials the original was written for, but breaks down here,
    # where this runs once over a multi-minute continuous LFP with many
    # flagged epochs recurring throughout it -- it would discard nearly the
    # whole session's phase after the first one. Unwrapping only the
    # trustworthy samples reconstructs each flagged epoch the same way
    # (pchip from the surrounding reliable phase) without that cascading,
    # whole-recording corruption.)
    valid = ~idx
    ...
    xgp = md * np.exp(1j * p)
    wt_post = _inst_freq(xgp)
    return xgp, wt_raw, wt_post
...
def plot_gp_instantaneous_frequency(wt_pre, wt_post, filter_band, title, out_path: Path,
    ...
    generalized_phase_vector's returned instantaneous frequency, recomputed
    from the phase-slip-stitched analytic signal), on the same figure so the
    effect of the correction is directly comparable.
...
def process_session(data_folder: Path, rng) -> list[dict]:
    ...
    filtered_lfp = bandpass_filter(lfp_sig, LFP_FILTER_BAND[0], LFP_FILTER_BAND[1], lfp_fs)
    xgp_lfp, lfp_inst_freq_pre, lfp_inst_freq_post = generalized_phase_vector(

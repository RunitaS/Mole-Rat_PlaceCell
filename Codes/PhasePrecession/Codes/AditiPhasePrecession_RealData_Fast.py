...
Data layout (unchanged from the original):
...
def process_session(data_folder: Path, rng) -> list[dict]:
    ...
    # Computed ONCE per session and reused for every unit and every shuffle
    # below -- this is the main performance fix relative to the original
    ...
    print(f'  vel={vel:.1f} cm/s, numpasses={numpasses}, posbins={len(posbins)} x '
          f'{POSBIN_SIZE_CM:.1f} cm, {len(pk_times)} theta peaks')

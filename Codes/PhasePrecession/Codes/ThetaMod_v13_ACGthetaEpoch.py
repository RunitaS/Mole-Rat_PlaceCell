...
window: p < ALPHA gates significance. A significant
...
#!alongside the existing Kempter asymptotic z-test classification, as a second, independent set of
...
ROOT_FOLDER = Path(r"C:/Runita/NMR/analysis/AllSort_Results/PlaceCell/Data/Debug/QT/Fa8477/Circle/Day5")
...
# Alternative to the Kempter asymptotic z-test classification above: a plain Pearson correlation
...
# fit + asymptotic z-test above, computed on the same spk_pass_index/
...
def compute_pass_index(pos_ts, pos_xy, spk_ts, lfp_ts, lfp_sig, lfp_fs, rng,
    ...
    # non-precessing and no further analysis (Wang classification) is run.
    n_fit_lines = np.nan
    fit_too_steep = False
    ...
    is_phase_locked = bool(np.isfinite(p) and not is_significant_precession)
    if not np.isfinite(p):
        ...
        precession_class = 'phase_processing'
    else:
    ...
    # Kempter asymptotic z-test classification above -- see classify_wang.
    if fit_too_steep:
        wang_results = dict(
            Wang_major_r=np.nan, Wang_major_p=np.nan, Wang_major_n=0, Wang_major_significant=False,
            Wang_minor_r=np.nan, Wang_minor_p=np.nan, Wang_minor_n=0, Wang_minor_significant=False,
            Wang_is_precessing=False, Wang_is_procession=False, Wang_class='non_precessing')
    else:
...
def assign_spike_phase(spk_ts, lfp_ts, filtered_lfp):
    ...
    before the first or after the last LFP peak."""
    return np.mod(peak_interp_phase(filtered_lfp, lfp_ts, spk_ts), 2 * np.pi)
...
def compute_theta_modulation(spk_ts, lfp_ts, filtered_lfp, rng):
    ...
    """
    phase_rad = assign_spike_phase(spk_ts, lfp_ts, filtered_lfp)
...
def process_session(data_folder: Path, rng) -> list[dict]:
    ...
    for ntt_path in ntt_files:
        ...
        for cell_number, spk_ts in units.items():
            ...
            metrics, phase_deg = compute_theta_modulation(spk_ts, lfp_ts, filtered_lfp, rng)
            row.update(metrics)
            ...
            if not metrics['TMI_Significant']:
                ...
                if len(spk_ts_overlap) < MIN_SPIKES_FOR_FIT:
                    ...
                    try:
                        ...
                        # Cells positive on BOTH Wang and Kempter asymptotic z-test (precessing):
                        # also copy the plot into ROOT_FOLDER/PrecessionPositive_Wang_and_Kempter
                        if results['is_precessing'] and results['Wang_is_precessing']:
                            pos_dir = ROOT_FOLDER / 'PrecessionPositive_Wang_and_Kempter'
                            pos_dir.mkdir(parents=True, exist_ok=True)
                            shutil.copy2(png_path, pos_dir / f'{session_label}_{unit_label}_PassIndex.png')

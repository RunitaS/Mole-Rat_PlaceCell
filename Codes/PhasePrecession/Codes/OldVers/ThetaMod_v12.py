...
unit (one .ntt file = one already-isolated unit):
  ...
  Step 1 (visualization only): polar plot of spike counts vs. instantaneous
    ...
    collapsed/negative instantaneous frequency. The GP correction is
    optional: set USE_GPA_CORRECTION = False below to use the plain
    Hilbert-transform instantaneous phase/frequency/amplitude everywhere
    instead (Steps 1-3 alike, including Step 3's spatial pass-index phase).
...
p_slope_shuffle, PrecessionShuffleMethod, PrecessionClass) where it ran.
Step 3's significance is a shuffle test, not a fixed slope-magnitude window,
and which of two null models is used for every unit is set by the
PRECESSION_SHUFFLE_METHOD config constant below: 'spike' (default) circularly
shifts spike times against the position/theta-phase traces many times
(shuffle_precession_significance); 'phase' instead randomizes each burst's
LFP theta phase while keeping spike times/pass index fixed
(shuffle_precession_significance_phase), the same burst-preserving
random-phase mechanism as Step 2's TMI shuffle test, reused here to save the
compute cost of re-interpolating the traces every shuffle. Either way the
null distribution is built from rho (and, as a secondary diagnostic, the
slope itself), and p_rho_shuffle < ALPHA gates significance. A significant
negative slope is labeled
...
A second, independent classification (Wang, Foster & Pfeiffer 2020, Science
370:247-250 -- classify_wang) is computed alongside it on the same
pass-index/theta-phase data, reported as the Wang_* columns
(Wang_major_r/p/n/significant, Wang_minor_r/p/n/significant,
Wang_is_precessing, Wang_is_procession, Wang_class): rather than fitting one
whole-pass circular-linear slope, it restricts spikes to a fixed theta-phase
window (major/forward or minor/reverse, boundaries from that paper's own
Fig. 2B) and tests a plain Pearson correlation between raw theta phase and
pass index within each window independently -- so, unlike PrecessionClass
above, a unit can be Wang_is_precessing and Wang_is_procession at once. Both
classifications are kept side by side (neither overwrites or gates the
other) so they can be compared against each other before picking one.

Per-unit plots (polar plot from Step 1, and the 6-panel Pass Index summary
from Step 3, when run -- its phase/pass-index panel also shades the Wang
major/minor windows and reports both classifications' stats) are saved to
...
#!Added Wang, Foster & Pfeiffer (2020, Science 370:247-250) classification (classify_wang)
#!alongside the existing Kempter-shuffle classification, as a second, independent set of
#!Wang_* columns, so both can be checked against each other before picking one.
...
OUTPUT_EXCEL_NAME = 'theta_phase_GPA_debug_PhaseShuffle.xlsx'   # written to ROOT_FOLDER

TRACKING_TIME_UNIT = 'us'     # 'us', 'ms', or 's' -- units of the tracking timestamp column

# --- Generalized Phase (GP) correction toggle ---
# True (default): every instantaneous phase/frequency estimate below (LFP theta phase
# for Steps 1-3, and the spatial pass-index phase in Step 3) is computed via the
# Generalized Phase correction (Davis, Muller et al. 2020) -- see the "Generalized
# Phase" section further down for what that correction does and why.
# False: skip the correction entirely and use the plain Hilbert-transform analytic
# signal instead (np.angle(signal.hilbert(x)), with no phase-slip detection/repair or
# rotation-direction rectification) everywhere generalized_phase_vector() is called.
USE_GPA_CORRECTION = False
...
PRECESSION_SHUFFLE_METHOD = 'phase'  # null model for Step 3's significance test (see compute_pass_index):
                                      # 'spike' circularly time-shifts spike times against the real
                                      # position/theta-phase traces (shuffle_precession_significance);
                                      # 'phase' instead randomizes each burst's LFP theta phase, keeping
                                      # spike times/pass index fixed (shuffle_precession_significance_phase,
                                      # the same mechanism as Step 2's TMI shuffle test)
N_PRECESSION_SHUFFLES = 1000    # shuffles testing the pass-index/theta-phase circular-linear fit
                                # (rho, slope) against a null of no position-phase relationship --
                                # see shuffle_precession_significance / shuffle_precession_significance_phase
PRECESSION_MIN_SHIFT_FRAC = 0.1  # minimum circular shift, as a fraction of the overlap window's
                                  ...
                                  # (only used by the 'spike' method)

# --- Step 3b: Wang, Foster & Pfeiffer (2020, Science 370:247-250) classification ---
# Alternative to the Kempter-shuffle classification above: a plain Pearson correlation
# between theta phase and pass index within a fixed phase window, one window per direction.
# Window boundaries are taken directly from that paper's Fig. 2B / Methods (population
# theta-phase troughs), not re-derived per unit -- see classify_wang.
WANG_MAJOR_WINDOW_DEG = (200.0, 430.0)  # forward window ("precession"); 430 wraps to 70 deg
WANG_MINOR_WINDOW_DEG = (80.0, 190.0)   # reverse window ("procession"); no wraparound
MIN_SPIKES_FOR_WANG_FIT = 10             # skip a window's Pearson fit if fewer spikes fall in it
...
def generalized_phase_vector(x, fs, lp, nwin=3, use_correction=None):
    ...
    use_correction : if True, run the GP phase-slip correction below; if
         False, skip it entirely and return the plain Hilbert-transform
         analytic signal (xo = signal.hilbert(x)) untouched -- no
         rotation-direction rectification, no phase-slip detection/repair,
         so np.angle/instantaneous-frequency/amplitude off the result are
         exactly the "pure Hilbert" values. Defaults to the module-level
         USE_GPA_CORRECTION toggle when None (the normal case -- pass this
         explicitly only to override that global for one call).

    Returns
    -------
    xgp     : complex analytic signal with the corrected ("generalized") phase,
         or the plain Hilbert analytic signal when use_correction is False.
    wt_pre  : instantaneous frequency of the RAW, pre-correction Hilbert
         ...
         artifact this function corrects. When use_correction is False this
         is simply xgp's own instantaneous frequency (nothing was corrected).
    idx     : boolean mask, True where the sample fell inside a phase-slip
         ...
         this reason. All False when use_correction is False.
    """
    if use_correction is None:
        use_correction = USE_GPA_CORRECTION
    ...
    if not use_correction:
        return xo, wt_raw, np.zeros(npts, dtype=bool)
...
def anglereg(x: np.ndarray, theta: np.ndarray, bnds=None):
    ...
    per shuffle in shuffle_precession_significance (N_PRECESSION_SHUFFLES=
    500 by default) -- the unvectorized loop dominated Step 3's runtime.
    """
...
def shuffle_precession_significance_phase(spk_ts, spk_pass_index, spk_theta_phase,
                                           observed_rho, observed_s, rng,
                                           n_shuffles=N_PRECESSION_SHUFFLES, slope_bnds=None):
    """Alternative null distribution for the pass-index/theta-phase
    circular-linear fit, shuffling LFP theta phase instead of spike time:
    uses the same burst-preserving random-phase mechanism as Step 2's TMI
    shuffle test (_burst_groups/shuffle_tmi_significance) -- consecutive
    spikes < TMI_BURST_MAX_GAP_SEC apart and in the same real-phase bin
    (TMI_BURST_PHASE_BIN_DEG wide) are kept together and assigned one
    independent random phase in [0, 360) per shuffle. spk_pass_index (the
    real, observed pass index at each spike) is left untouched; only the
    theta phase paired with it is randomized, destroying the cell-specific
    spike-phase relationship while preserving burst structure and the real
    position trace -- analogous to Step 2's TMI shuffle test, but applied to
    Step 3's spike subset (spk_ts, the tracking/LFP overlap window) and its
    own real theta phase (spk_theta_phase), not Step 2's.

    Much cheaper per iteration than the circular time-shift shuffle
    (shuffle_precession_significance) above -- a phase draw and a refit, no
    re-interpolation of the position/LFP traces -- so it reuses
    N_PRECESSION_SHUFFLES rather than needing its own iteration count.

    Returns (p_rho, p_slope, shuffle_rhos, shuffle_slopes), same convention
    as shuffle_precession_significance.
    """
    phase_deg = np.degrees(np.mod(spk_theta_phase, 2 * np.pi))
    ...
    shuffle_slopes = np.full(n_shuffles, np.nan)
    for i in range(n_shuffles):
        group_phases_deg = rng.uniform(0.0, 360.0, size=n_groups)
        shuffled_phase_rad = np.radians(group_phases_deg[group_id])

        rho_i, _p_i, s_i, _b_i = kempter_lincirc(spk_pass_index, shuffled_phase_rad,
                                                  slope_bnds=slope_bnds)
        shuffle_rhos[i] = rho_i
        shuffle_slopes[i] = s_i
...
# ============================================================================
# Wang, Foster & Pfeiffer (2020, Science 370:247-250) phase precession/
# procession classification -- an alternative to the Kempter circular-linear
# fit + shuffle test above, computed on the same spk_pass_index/
# spk_theta_phase for direct comparison. Restricts spikes to a fixed
# theta-phase window (forward/major or reverse/minor; boundaries from that
# paper's Fig. 2B/Methods, not re-derived per unit or per session here) and
# tests a plain (non-circular) Pearson correlation between raw theta phase
# and pass index within that window -- valid without circular statistics
# only because each window is chosen so phase does not wrap in the middle
# of it (any spike whose phase falls in the low-degree tail of a wrapping
# window, e.g. the major window's 0-70 deg tail, has 360 added so the whole
# window's phase is one continuous, increasing scale for the correlation).
# Unlike the Kempter-based classification, the major/minor windows are
# tested independently, so a unit can be significant in both (Wang et al.'s
# central finding for many bimodal cells).
# ============================================================================

def wang_window_correlation(pass_index, theta_phase_rad, window_deg):
    """Pearson correlation between theta phase and pass index within one
    fixed phase window (Wang et al. 2020). Returns dict(r, p, n); r and p
    are NaN if fewer than MIN_SPIKES_FOR_WANG_FIT spikes fall in the window
    (or the window's spikes have no spread in phase or pass index), rather
    than raising, so a sparse/degenerate unit just gets an undefined
    result instead of aborting the whole classification.
    """
    lo_deg, hi_deg = window_deg
    phase_deg = np.degrees(np.mod(theta_phase_rad, 2 * np.pi))

    if hi_deg <= 360.0:
        mask = (phase_deg >= lo_deg) & (phase_deg <= hi_deg)
        phase_unwrapped = phase_deg
    else:
        hi_wrapped = hi_deg - 360.0
        mask = (phase_deg >= lo_deg) | (phase_deg <= hi_wrapped)
        # only the wrapped tail (phase < lo_deg) needs +360 so the window's
        # phase is one continuous increasing scale for the correlation
        phase_unwrapped = np.where(phase_deg < lo_deg, phase_deg + 360.0, phase_deg)

    x, y = np.asarray(pass_index)[mask], phase_unwrapped[mask]
    good = np.isfinite(x) & np.isfinite(y)
    x, y = x[good], y[good]
    n = len(x)
    if n < MIN_SPIKES_FOR_WANG_FIT or np.ptp(x) == 0 or np.ptp(y) == 0:
...
def classify_wang(pass_index, theta_phase_rad, alpha=ALPHA):
    """Wang, Foster & Pfeiffer (2020) classification: a significant negative
    phase/pass-index correlation in the major (forward) window is
    phase-precessing; a significant positive correlation in the minor
    (reverse) window is phase-procession (that paper's term -- the analogue
    of "recessing" elsewhere in this module, but tested in a disjoint phase
    window rather than as the opposite sign of one whole-pass fit)."""
    major = wang_window_correlation(pass_index, theta_phase_rad, WANG_MAJOR_WINDOW_DEG)
    minor = wang_window_correlation(pass_index, theta_phase_rad, WANG_MINOR_WINDOW_DEG)

    major_sig = bool(np.isfinite(major['p']) and major['p'] < alpha)
    minor_sig = bool(np.isfinite(minor['p']) and minor['p'] < alpha)
    is_precessing = bool(major_sig and major['r'] < 0)
    is_procession = bool(minor_sig and minor['r'] > 0)

    if is_precessing and is_procession:
        wang_class = 'both'
    elif is_precessing:
        wang_class = 'phase_precessing'
    elif is_procession:
        wang_class = 'phase_procession'
    elif major_sig or minor_sig:
        # significant but wrong-signed for either of Wang et al.'s two
        # positive categories (e.g. a significant positive major-window
        # correlation) -- kept distinct from the untested/flat case below.
        wang_class = 'other_significant'
    else:
        wang_class = 'phase_locked'

    return dict(
        Wang_major_r=major['r'], Wang_major_p=major['p'], Wang_major_n=major['n'],
        Wang_major_significant=major_sig,
        Wang_minor_r=minor['r'], Wang_minor_p=minor['p'], Wang_minor_n=minor['n'],
        Wang_minor_significant=minor_sig,
        Wang_is_precessing=is_precessing, Wang_is_procession=is_procession,
...
def compute_pass_index(pos_ts, pos_xy, spk_ts, lfp_ts, lfp_sig, lfp_fs, rng,
                        ...
                        slope_bnds=None, precession_shuffle_method='spike'):
    ...
    # Significance via shuffle test rather than a fixed slope-magnitude window:
    # a real but shallow precession should not be discarded just for falling
    # outside an arbitrary deg/pass range, and kempter_lincirc's own asymptotic
    # p (still reported below as 'p', for QC) assumes large-n normality that
    # real spike trains can violate. Two null models are available, chosen for
    # the whole run by the PRECESSION_SHUFFLE_METHOD config constant:
    # 'spike' circularly time-shifts spike times against the real position/
    # theta-phase traces (shuffle_precession_significance); 'phase' instead
    # randomizes each burst's LFP theta phase, keeping spike times/pass index
    # fixed (shuffle_precession_significance_phase, the same mechanism as Step
    # 2's TMI shuffle test).
    if precession_shuffle_method == 'phase':
        p_rho_shuffle, p_slope_shuffle, _shuffle_rhos, _shuffle_slopes = shuffle_precession_significance_phase(
            spk_ts, spk_pass_index, spk_theta_phase, rho, s, rng, slope_bnds=slope_bnds)
    else:
    ...
    # Second, independent classification (Wang et al. 2020) on the same
    # spk_pass_index/spk_theta_phase, for direct comparison against the
    # Kempter-shuffle classification above -- see classify_wang.
    wang_results = classify_wang(spk_pass_index, spk_theta_phase)
    ...
    return {
        ...
        'precession_shuffle_method': precession_shuffle_method,
...
def plot_unit_summary(pos_xy, results, title, out_path: Path):
    ...
    ax = axes[1, 0]
    # Wang et al. 2020 windows, shaded behind the scatter for visual comparison
    # against the Kempter fit line -- shown at both their base position and
    # +360 deg, matching the doubled (0-720 deg) phase axis used below.
    maj_lo, maj_hi = WANG_MAJOR_WINDOW_DEG
    min_lo, min_hi = WANG_MINOR_WINDOW_DEG
    for lo, hi in ((maj_lo, maj_hi), (maj_lo + 360, maj_hi + 360)):
        ax.axhspan(max(lo, 0), min(hi, 720), color='tab:blue', alpha=0.08, zorder=0)
    for lo, hi in ((min_lo, min_hi), (min_lo + 360, min_hi + 360)):
        ax.axhspan(max(lo, 0), min(hi, 720), color='tab:orange', alpha=0.08, zorder=0)
    ...
    ax.scatter(pi_dup, phase_dup, s=8, alpha=0.6, zorder=2)
    ...
    ax.set_title(f'rho={results["rho"]:.2f}  p_shuffle={results["p_rho_shuffle"]:.3g}  '
                 ...
                 f'Wang: major r={results["Wang_major_r"]:.2f} (p={results["Wang_major_p"]:.3g})  '
                 f'minor r={results["Wang_minor_r"]:.2f} (p={results["Wang_minor_p"]:.3g})  '
                 f'class={results["Wang_class"]}', fontsize=9)
    ...
    text_lines = [
        ...
        '-- Kempter (circular-linear) + shuffle --',
        f"rho = {results['rho']:.3f}",
        f"p (asymptotic) = {results['p']:.4g}",
        f"slope = {results['slope_deg_per_pass']:.2f} deg/pass",
        f"p_rho_shuffle = {results['p_rho_shuffle']:.4g}",
        f"p_slope_shuffle = {results['p_slope_shuffle']:.4g}",
        f"class = {results['precession_class']}",
        '-- Wang et al. 2020 (windowed Pearson) --',
        f"major window r={results['Wang_major_r']:.3f}  p={results['Wang_major_p']:.4g}  "
        f"n={results['Wang_major_n']} (sig={results['Wang_major_significant']})",
        f"minor window r={results['Wang_minor_r']:.3f}  p={results['Wang_minor_p']:.4g}  "
        f"n={results['Wang_minor_n']} (sig={results['Wang_minor_significant']})",
        f"class = {results['Wang_class']}",
    ]
...
def process_session(data_folder: Path, rng, precession_shuffle_method='spike') -> list[dict]:
    ...
    for ntt_path in ntt_files:
        ...
        if theta_ncs not in lfp_cache:
            ...
            if USE_GPA_CORRECTION:
                ...
                print('  USE_GPA_CORRECTION=False -- using plain Hilbert-transform phase/frequency '
                      '(GP correction and its before/after debug plot skipped).')
                ...
                if len(spk_ts_overlap) < MIN_SPIKES_FOR_FIT:
                    ...
                    try:
                        ...
                        row.update({
                            ...
                            'PrecessionShuffleMethod': results['precession_shuffle_method'],
                            ...
                            'Wang_major_r': results['Wang_major_r'],
                            'Wang_major_p': results['Wang_major_p'],
                            'Wang_major_n': results['Wang_major_n'],
                            'Wang_major_significant': results['Wang_major_significant'],
                            'Wang_minor_r': results['Wang_minor_r'],
                            'Wang_minor_p': results['Wang_minor_p'],
                            'Wang_minor_n': results['Wang_minor_n'],
                            'Wang_minor_significant': results['Wang_minor_significant'],
                            'Wang_is_precessing': results['Wang_is_precessing'],
                            'Wang_is_procession': results['Wang_is_procession'],
                            'Wang_class': results['Wang_class'],
                        })
                        print(f'  {unit_label}: PRECESSION rho={results["rho"]:.3f}  '
                              f'p_rho_shuffle={results["p_rho_shuffle"]:.3g}  '
                              f'slope={results["slope_deg_per_pass"]:.1f} deg/pass  '
                              f'class={results["precession_class"]}  '
                              f'Wang_class={results["Wang_class"]}')
...
def build_summary_stats(df: pd.DataFrame) -> pd.DataFrame:
    ...
    n_wang_precessing = int((df['Wang_is_precessing'] == True).sum())        # noqa: E712
    n_wang_procession = int((df['Wang_is_procession'] == True).sum())        # noqa: E712
    n_wang_locked = int((df['Wang_class'] == 'phase_locked').sum())
    ...
    rows = [
        ...
        dict(Metric='Wang_is_precessing cells (of precession-tested cells)',
             Count=n_wang_precessing, Denominator=n_precession_tested,
             DenominatorLabel='precession-tested cells',
             Percent=pct(n_wang_precessing, n_precession_tested)),
        dict(Metric='Wang_is_procession cells (of precession-tested cells)',
             Count=n_wang_procession, Denominator=n_precession_tested,
             DenominatorLabel='precession-tested cells',
             Percent=pct(n_wang_procession, n_precession_tested)),
        dict(Metric='Wang phase_locked cells (of precession-tested cells)',
             Count=n_wang_locked, Denominator=n_precession_tested,
             DenominatorLabel='precession-tested cells',
             Percent=pct(n_wang_locked, n_precession_tested)),
    ]
...
def main():
    ...
    if PRECESSION_SHUFFLE_METHOD not in ('spike', 'phase'):
        raise ValueError("PRECESSION_SHUFFLE_METHOD must be 'spike' or 'phase'.")

    precession_shuffle_method = PRECESSION_SHUFFLE_METHOD
    print(f'Phase-precession shuffle test: PRECESSION_SHUFFLE_METHOD={precession_shuffle_method!r}')
    ...
    for data_folder in session_folders:
        print(f'\n=== Session: {data_folder} ===')
        try:
            all_rows.extend(process_session(data_folder, rng, precession_shuffle_method))
        except Exception as exc:
    ...
    columns = ['Session', 'FolderPath', 'Unit', 'ntt_file', 'lfp_file', 'cell_number', 'n_spikes_total',
               ...
               'PrecessionShuffleMethod', 'slope_deg_per_pass', 'phase_range_deg',
               'is_precessing', 'is_recessing', 'is_phase_locked', 'PrecessionClass',
               'Wang_major_r', 'Wang_major_p', 'Wang_major_n', 'Wang_major_significant',
               'Wang_minor_r', 'Wang_minor_p', 'Wang_minor_n', 'Wang_minor_significant',
               'Wang_is_precessing', 'Wang_is_procession', 'Wang_class',
